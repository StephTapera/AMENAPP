...
class AntiHarassmentEngine {
    ...
    func recordEnforcement(
        ...
        if let existing, !existing.documents.isEmpty {
            dlog("⚠️ [ENFORCEMENT] Duplicate suppressed for idempotency key: \(idempotencyKey)")
            return
        }
        ...
        dlog("📝 [ENFORCEMENT] Recorded: User \(userId), Violation: \(violation), Action: \(action), Source: \(source.rawValue), Surface: \(surface.rawValue)")
    }
    ...
    func enableUserProtection(userId: String, reason: String, expiresAfterDays: Int = 7) async throws {
        ...
        if let existing, existing.exists,
           ...
           existingExpiry > expiresAt {
            dlog("ℹ️ [PROTECTION] Already active for \(userId) until \(existingExpiry) — not shortening.")
            return
        }
        ...
        dlog("🛡️ [PROTECTION] Enabled for user: \(userId), Reason: \(reason), Expires: \(expiresAt)")
    }
    ...
    func applyRestriction(
        ...
        if let existing, existing.exists,
           ...
           existingEnd > proposedEndDate {
            dlog("ℹ️ [RESTRICTION] Existing restriction for \(userId)/\(type.rawValue) ends later (\(existingEnd)) — not shortening.")
            return
        }
        ...
        dlog("⏸️ [RESTRICTION] Applied: User \(userId), Type: \(type.rawValue), Duration: \(durationHours)h")
    }
    ...
    func submitAppeal(
        ...
        dlog("📝 [APPEAL] Submitted: \(appealId), User: \(userId)")
    ...
    func handleHarassmentPattern(_ pattern: HarassmentPattern) async throws {
        dlog("🚨 [PATTERN] Harassment detected: \(pattern.userId) → \(pattern.targetUserId), Risk: \(pattern.riskLevel)")
        ...
        if hasMinorSafetyViolation {
            dlog("🚨 [PATTERN] MINOR-SAFETY override — applying critical path regardless of risk level")
        ...
        case .low:
            ...
            dlog("⚠️ [WARNING] Sent to user: \(pattern.userId)")
        }
    }
}
