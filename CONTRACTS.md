...
> NOTE: This swarm (Context System — this conversation) is the SOLE canonical builder.
> A duplicate mission was terminated 2026-06-10 after it truncated `ContextStoreModels.swift`
> to 2 lines; that file was restored from this contract. See `AMENAPP/ContextStore/RUNLOG.md`.
...
(NOTE: repo has no Firestore emulator harness yet — rules asserted by text-invariant + unit
guards until a human stands up the emulator).

## 7. Cloud Functions (signatures frozen)
All `onCall`, `enforceAppCheck: true`, region `us-central1`, project `amen-5e359`.
Tier-P inputs/outputs forbidden in every signature below. TODO(cf) placeholders do NOT ship
past Wave 3 — they join the Stage-3 deploy batch when built.
