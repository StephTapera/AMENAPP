...
struct PostCardRenderModelTests {
    ...
    // MARK: 8. Author verification state

    @Test("authorIsVerified is false when verificationType is .none")
    func authorNotVerifiedByDefault() {
        let model = PostCardRenderModel.preview(authorVerificationType: .none)
        #expect(!model.authorIsVerified)
        #expect(model.authorVerificationType == .none)
    }

    @Test("authorIsVerified is true for church verificationType")
    func authorVerifiedChurch() {
        let model = PostCardRenderModel.preview(authorVerificationType: .church)
        #expect(model.authorIsVerified)
        #expect(model.authorVerificationType == .church)
    }

    @Test("authorIsVerified is true for business verificationType")
    func authorVerifiedBusiness() {
        let model = PostCardRenderModel.preview(authorVerificationType: .business)
        #expect(model.authorIsVerified)
