...
/// Pure SwiftUI ultra-thin material — replaces the former UIVisualEffectView bridge.
/// SwiftUI's native material handles view lifecycle correctly across tab transitions,
/// avoiding the EXC_BAD_ACCESS (CALayerGetSuperlayer at 0x8) crash that occurred
/// when a UIViewRepresentable layer was committed after its host was torn down.
/// The .mask() in DynamicGlassOverlay works identically with SwiftUI-native materials.
private struct GlassMaterialBlurView: View {
    // colorScheme parameter kept so the call site in DynamicGlassOverlay is unchanged.
    // SwiftUI's .ultraThinMaterial adapts automatically to dark/light mode.
        ...
        GeometryReader { geo in
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    ...
                    GlassMaterialBlurView(colorScheme: colorScheme)
