...
echo "Configuring Universal Creation Platform secrets for Firebase project: $PROJECT_ID"
echo "Secret values are entered through the Firebase CLI and are not written to this repo."
echo ""

"${FIREBASE_CMD[@]}" --project "$PROJECT_ID" functions:secrets:set ANTHROPIC_API_KEY
"${FIREBASE_CMD[@]}" --project "$PROJECT_ID" functions:secrets:set OPENAI_API_KEY

echo ""
echo "Verifying Firebase Functions can access required secrets..."
...
echo ""
echo "Universal Creation Platform secret configuration complete."
echo "Next run:"
echo "  FIREBASE_PROJECT_ID=$PROJECT_ID REQUIRE_FIREBASE_LIVE=1 scripts/verify_universal_platform_go_live.sh"
