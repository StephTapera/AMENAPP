...
> 🧭 **GLOBAL CLEAN-CODE REVIEW (2026-06-11): EVERY IMPORT MUST RESOLVE.** Incident class: a build log reported invented module import `AccessibilityPrivate` in `AgeVerificationOnboardingView.swift`; current tree has no such import, `git log -S/-G AccessibilityPrivate -- AMENAPP/AgeVerificationOnboardingView.swift` returns no lane commit, and `BuildProject` is green. Pre-commit self-review now includes checking new imports against public SDK/package modules; nonexistent modules are lane API invention bugs.
>
...
| **claude — Connect Discovery Engine** | `AMENAPP/AMENAPP/DiscoveryOS/**` (all new files), `Backend/functions/src/discovery/**`, `docs/DISCOVERY_SCHEMA.md`, `docs/DISCOVERY_TELEMETRY.md`, `AMENAPP/AMENAPP/AMENFeatureFlags.swift` (append-only System 45 block), `Backend/functions/src/index.ts` (append-only exports), `firestore.indexes.json` (append-only discovery indexes) | 2026-06-14 | ACTIVE — Wave 0 contracts frozen; Waves 1-3 building |
