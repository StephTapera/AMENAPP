...
struct DetectedScriptureRef: Codable {
...
struct FaithIntelResult: Codable {
    ...
    let detectedRefs: [DetectedScriptureRef]
...
actor FaithIntelService {
    ...
    /// Resolves a single raw scripture reference string (e.g. "John 3:16") to a `DetectedScriptureRef`.
    /// Returns nil if the proxy cannot resolve the reference.
    func resolveReference(_ rawRef: String) async throws -> DetectedScriptureRef? {
    ...
    private func parseRefs(from value: Any?) -> [DetectedScriptureRef] {
        guard let array = value as? [[String: Any]] else { return [] }
        return array.compactMap { parseRef(from: $0) }
    }

    private func parseRef(from dict: [String: Any]) -> DetectedScriptureRef? {
        guard
