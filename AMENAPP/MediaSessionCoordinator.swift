...
final class MediaSessionCoordinator: ObservableObject {
    ...
    private let syncInterval: TimeInterval = 10.0 // Write every 10s
    ...
    func beginSession(
        ...
        // If switching to a different video, save the old one first
        ...
        // Load cached state or Firestore state to resume
    ...
    /// Get the cached resume state for a video (nil if not started).
    func resumeState(for postId: String, mediaItemId: String) -> MediaPlaybackState? {
        let key = "\(postId)_\(mediaItemId)"
        return stateCache[key]
    }

    /// Preload resume states from Firestore for feed display.
