...
final class AMENFeatureFlags: ObservableObject {
    ...
    /// Default ON as of 2026-06-22 — the a11y (VoiceOver/Dynamic Type/Reduce Motion+
    /// Transparency) and scroll-perf verification matrix passed. When false, host screens
    /// (e.g. ActivityFeedView) render their existing layout unchanged — zero regression.
    /// Remote Config `collapsible_glass_header_enabled` still overrides this and remains
    /// the server-side kill switch.
    @Published private(set) var collapsibleGlassHeaderEnabled: Bool = true
    ...
    /// LIVE: each section is generated by the real Berean Constitutional Pipeline
    ...
    /// LIVE: real Firestore communityReflections read/write; submissions pass through
    /// ContentModerationService and persist as .pending until approved server-side.
    @Published private(set) var creatorTestimonyEnabled: Bool = true
    ...
    /// LIVE: insights computed by `generateStudioInsights` from the creator's real
    /// creatorProjects and read from creatorStudioInsights (backend deploy required).
    @Published private(set) var creatorStudioDashboardEnabled: Bool = true
    ...
    /// LIVE: depth is threaded end-to-end — BereanQuery.depth → bereanPipeline drives
    /// model tier, max_tokens, evidence breadth, and prompt posture (backend deploy required).
    @Published private(set) var longPressDepthDialEnabled: Bool = true
    ...
    private func buildDefaults() -> [String: NSObject] {
        [
            ...
            // Collapsible Liquid Glass Header (default ON — verification matrix passed 2026-06-22)
            "collapsible_glass_header_enabled": true as NSObject,
