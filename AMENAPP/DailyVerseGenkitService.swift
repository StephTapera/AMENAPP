...
class DailyVerseGenkitService: ObservableObject {
    ...
    // OFFLINE FIX: true when todayVerse was generated by the AI Cloud Function;
    // false when it came from the curated fallback rotation (network unavailable).
    // DailyVerseBanner uses this to retry once network is restored so users who
    // opened the app offline still receive a personalised verse during the day.
