...
struct AMENTabBar: View {
    ...
    private var nativeBody: some View {
        ZStack(alignment: .bottomTrailing) {
            NativeLiquidGlassContainer(spacing: 10) {
                HStack(spacing: 0) {
                    ForEach(AMENTab.visibleTabs, id: \.rawValue) { tab in
                        nativeTabItem(tab)
                            .frame(maxWidth: .infinity)
...
extension View {
    func amenTabBarScrollTracking(coordinateSpaceName: String = "amenScroll") -> some View {
        modifier(AMENScrollTrackingModifier(coordinateSpaceName: coordinateSpaceName))
    }
}
