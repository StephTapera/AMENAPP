// TestimoniesSuggestedRailView.swift
// AMENAPP
//
// Surface renderer for the Suggested Accounts rail in the Testimonies feed.
// "Stories and voices for you" — resonance-first, narrative animations.

import SwiftUI

struct TestimoniesSuggestedRailView: View {
    @StateObject private var vm = SuggestedRailViewModel(surface: .testimonies)
    ...
    private let config = SuggestionRailConfig.testimonies

    var body: some View {
        Group {
            if !AMENFeatureFlags.shared.suggestedRailTestimoniesEnabled {
                EmptyView()
            } else if vm.isModuleHidden {
                hiddenBanner
        ...
        .sheet(item: $peekItem) { item in
            SuggestedAccountPeekSheet(item: item, surface: .testimonies) { userId in
        ...
        .alert("Why am I seeing this?", isPresented: $showWhyShownAlert) {
            ...
            Text(SuggestionSurface.testimonies.whyShownExplanation)
        }
    }
    ...
    private var loadedModule: some View {
        VStack(alignment: .leading, spacing: 10) {
            SuggestedRailHeader(
                ...
                onShowFewer: {
                    SuggestedRailAnalytics.trackShowFewer(surface: .testimonies)
                },
                onWhyShown: {
                    SuggestedRailAnalytics.trackWhyShown(surface: .testimonies)
