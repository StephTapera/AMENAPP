...
enum AMENAgeAssuranceTier: String, Codable {
...
enum AMENAgeVerificationStatus: String, Codable {
...
struct UserAgeProfile: Codable {
    ...
    /// CRITICAL-2 FIX: Must be var so requestAgeChange() can update it;
    /// using let caused the DOB to remain unchanged on tier updates,
    /// making the computed `age` property inconsistent with the stored tier.
    var dateOfBirth: Date
    
    /// Current age tier (derived from DOB or verification)
    var tier: AMENAgeAssuranceTier
    
    /// Verification status
    var verificationStatus: AMENAgeVerificationStatus
...
struct AgeVerificationEvent: Codable {
    ...
    let previousTier: AMENAgeAssuranceTier?
    let newTier: AMENAgeAssuranceTier?
    let success: Bool
    ...
    init(
        ...
        previousTier: AMENAgeAssuranceTier? = nil,
        newTier: AMENAgeAssuranceTier? = nil,
        success: Bool = true,
