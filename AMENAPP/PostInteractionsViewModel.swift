...
class PostInteractionsViewModel: ObservableObject {
    ...
    func toggleCommentAmen(_ commentId: String, postId: String, currentlyAmened: Bool) async {
        do {
            try await commentService.toggleAmen(commentId: commentId, postId: postId, currentlyAmened: currentlyAmened)
        } catch {
            handleError(error)
        }
    }
