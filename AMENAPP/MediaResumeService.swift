...
final class MediaResumeService {
    ...
    /// Persist a playback state to Firestore (debounced by caller).
    ...
    /// Load a single resume state for a specific video.
    ...
    /// Load all recent resume states (for showing resume pills in feed).
    ...
    /// Mark a state as completed (finished watching).
