// PrayerSuggestedRailView.swift
// AMENAPP
//
// Surface renderer for the Suggested Accounts rail in the Prayer feed.
// "Prayer connections" — care-first, calmer animations.

import SwiftUI

struct PrayerSuggestedRailView: View {
    @StateObject private var vm = SuggestedRailViewModel(surface: .prayer)
    ...
    private let config = SuggestionRailConfig.prayer

    var body: some View {
        Group {
            if !AMENFeatureFlags.shared.suggestedRailPrayerEnabled {
                EmptyView()
        ...
        .sheet(item: $peekItem) { item in
            SuggestedAccountPeekSheet(item: item, surface: .prayer) { userId in
    ...
    private var loadedModule: some View {
        VStack(alignment: .leading, spacing: 10) {
            SuggestedRailHeader(
                ...
                onShowFewer: {
                    SuggestedRailAnalytics.trackShowFewer(surface: .prayer)
                },
                onWhyShown: {
                    SuggestedRailAnalytics.trackWhyShown(surface: .prayer)
