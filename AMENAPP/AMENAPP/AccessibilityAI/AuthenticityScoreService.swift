...
actor AuthenticityScoreService {
    ...
    func computeScore(for mediaId: String) async throws -> MediaAuthenticityScore {
    ...
    func localFallback(for credential: C2PAMediaCredential) -> MediaAuthenticityScore {
        let originalCapture  = credential.originState == .verifiedOriginal
        let provenanceIntact = credential.c2paManifestPresent && credential.metadataIntact
        let editsDisclosed   = !credential.editChain.isEmpty || credential.originState != .edited

        return MediaAuthenticityScore(
    ...
    private func parseScore(from data: [String: Any]) throws -> MediaAuthenticityScore {
        guard
    ...
    private func offlineFallback() -> MediaAuthenticityScore {
        MediaAuthenticityScore(
