...
struct UploadManifestPreserver {
    ...
    /// Serialises a C2PAMediaCredential as JSON and writes it into the Storage
    /// object's custom metadata under the `amen_provenance` key.
    /// Call this before initiating a `putData` or `putFile` upload task.
    static func stamp(metadata: inout StorageMetadata, credential: C2PAMediaCredential) {
    ...
    static func extractCredentialStub(from metadata: StorageMetadata) -> C2PAMediaCredential? {
        guard let json = metadata.customMetadata?[metadataKey],
              let data = json.data(using: .utf8) else {
            return nil
        }

        return try? JSONDecoder().decode(C2PAMediaCredential.self, from: data)
    }
    ...
    /// - The origin state is something other than `.unverified`
    static func verifyManifestIntact(_ credential: C2PAMediaCredential) -> Bool {
        credential.c2paManifestPresent
            && credential.metadataIntact
            && credential.originState != .unverified
    }
}
