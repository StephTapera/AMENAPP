...
actor ProvenanceCredentialService {
    ...
    /// Registers a new C2PAMediaCredential by calling the `registerMediaProvenance`
    ...
    func registerCredential(_ credential: C2PAMediaCredential) async throws {
        ...
        let params: [String: Any] = [
            ...
            "state":          credential.originState.rawValue,
    ...
    /// Reads a C2PAMediaCredential from Firestore. Returns nil when no document exists.
    func fetchCredential(mediaId: String) async throws -> C2PAMediaCredential? {
        let snapshot = try await credentialDoc(mediaId: mediaId).getDocument()
        guard snapshot.exists else { return nil }

        do {
            return try snapshot.data(as: C2PAMediaCredential.self)
        } catch {
    ...
    /// Appends a C2PAAIContribution to the aiContributions array in Firestore.
    func attachAIContribution(_ contribution: C2PAAIContribution, to mediaId: String) async throws {
        guard Auth.auth().currentUser != nil else {
            throw ProvenanceCredentialError.unauthenticated
        }
