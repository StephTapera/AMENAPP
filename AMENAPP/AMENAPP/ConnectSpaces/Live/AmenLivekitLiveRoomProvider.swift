...
//
// Targets LiveKit client-sdk-swift >= 2.0.0
// SPM: https://github.com/livekit/client-sdk-swift
//
// Token + server URL come from the getLivekitToken Firebase callable.
// Set LIVEKIT_API_KEY, LIVEKIT_API_SECRET, LIVEKIT_URL in Firebase secrets:
...
final class AmenLivekitLiveRoomProvider: NSObject, AmenLiveRoomProvider, ObservableObject {
    ...
    func joinRoom(roomId: String, userId: String, displayName: String, mode: AmenLiveRoomMode) async throws {
        let fn = Functions.functions().httpsCallable("getLivekitToken")
    ...
    func muteParticipant(userId: String) async throws {
        // LiveKit server-side mute requires the server SDK. We signal via Firebase;
        // the CF writes isMuted to Firestore and the participant's device honors it.
    ...
    var participantVideoView: (String) -> AnyView {
        { [weak self] participantId in
            AnyView(RemoteLivekitVideoView(room: self?.room, participantIdentity: participantId))
        }
    }
...
extension AmenLivekitLiveRoomProvider: RoomDelegate {
...
private struct RemoteLivekitVideoView: View {
    ...
    private var remoteParticipant: RemoteParticipant? {
        room?.remoteParticipants.values.first { "\($0.identity)" == participantIdentity }
    }
    ...
    var body: some View {
        if let participant = remoteParticipant,
           let track = participant.firstCameraVideoTrack {
            ...
            RemoteParticipantPlaceholder(identity: participantIdentity)
        }
    }
}
