// AmenModerationDashboardView.swift
...
@MainActor
private final class AmenModerationDashboardViewModel: ObservableObject {
...
struct AmenModerationDashboardView: View {
    ...
    @StateObject private var viewModel: AmenModerationDashboardViewModel
    @State private var selectedTab: DashboardTab = .reports

    init(spaceId: String, isHost: Bool) {
        self.spaceId = spaceId
        self.isHost = isHost
        _viewModel = StateObject(wrappedValue: AmenModerationDashboardViewModel(spaceId: spaceId))
    }
...
#Preview("Host view") {
    AmenModerationDashboardView(spaceId: "s1", isHost: true)
        .frame(height: 600)
        .preferredColorScheme(.dark)
}

#Preview("Mod view") {
    AmenModerationDashboardView(spaceId: "s1", isHost: false)
        .frame(height: 600)
        .preferredColorScheme(.dark)
}
