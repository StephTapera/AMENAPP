...
struct AmenConnectSpacesFirestoreBinding {
    ...
    static func bindConnectVideo(_ snapshot: DocumentSnapshot) throws -> AmenConnectSpacesConnectVideo {
        let data = try data(from: snapshot)
        // Use explicit closures instead of method references to avoid
        // actor-isolation warnings on nonisolated private static methods.
        return AmenConnectSpacesConnectVideo(
            ...
            claims: try dictionaryArray(data, "claims").map { try teachingClaim($0) },
            scriptureRefs: try dictionaryArray(data, "scriptureRefs").map { try scriptureRef($0) },
    ...
    // Firestore.Encoder().encode(_:) returns [String: Any] directly;
    // the conditional cast always succeeds and was removed to silence the warning.
...
final class AmenConnectSpacesCallableProxy {
    ...
    func searchMinistryMemory(spaceId: String, query: String, limit: Int = 10) async throws -> [AmenConnectSpacesMinistryMemoryResult] {
        ...
        // Use explicit closure to avoid actor-isolation warning on the nonisolated static method reference.
        return try rows.map { try AmenConnectSpacesAegisBinding.ministryMemoryResult(from: $0) }
    }
...
struct AmenConnectSpacesAegisBinding {
    ...
    static func gateDecision(from data: [String: Any]) throws -> AmenConnectSpacesAegisGateDecision {
        ...
        // Use explicit closure to avoid actor-isolation warning on the nonisolated private static method reference.
        let flags = try (data["flags"] as? [[String: Any]] ?? []).map { try aegisFlag($0) }
...
// MARK: - HTTPSCallable Firebase v6 Timeout Extension

private extension HTTPSCallable {
    /// Routes a call through Firebase Functions v6 with the supplied timeout.
    /// Firebase Functions v6 requires `timeoutInterval` to be set on the callable
    /// instance before dispatch; this extension encapsulates that contract.
    func safeCall(_ data: [String: Any], timeout: TimeInterval) async throws -> HTTPSCallableResult {
        // `timeoutInterval` is a stored var on the HTTPSCallable struct, so we
        // copy `self`, mutate the copy, then call through it.  The local binding
        // is named `request` (not `callable`) so the compiler can confirm the
        // mutation via the property setter without a "never mutated" diagnostic.
        let request = self
        request.timeoutInterval = timeout
        return try await request.call(data)
    }
}

// MARK: - Firestore Conviction Check Binding
