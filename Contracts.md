...
Fires when a card is at least 50% visible for 1 continuous second, not when data loads.
...
| `visibilityThreshold` | number | Must be `0.5` for this release. |
...
- Client source: Firebase Remote Config key `banner_rail_enabled`, default `false`.
- Server source: both `process.env.BANNER_RAIL_ENABLED == "true"` and `system/serverFeatureFlags.bannerRail == true` must be true.
- Production writes and callable exposure remain inert while the flag is off.
- First production wiring is `spacesHome` only.

## Locked Smart Defaults

- `banner_cta_complete` fires only after the real downstream action succeeds. It must not fire on tap, route validation, or screen-open alone.
- Moderation approval is server-authoritative. Space/church admins may submit or edit pending banners, but only Cloud Functions or platform service code may set `moderationStatus = "approved"`.
- First rollout surface is `spacesHome` only. Other surfaces may compile against the same types but must remain unwired until QA signs off.
- Impression threshold is 50% visible for 1 continuous second.
- Surface defaults are `homeFeed = compact`; `spaceDetail`, `churchProfile`, `schoolProfile`, and `businessProfile = large`; all other surfaces = `standard`.
- If a CTA route type cannot be mapped to a real existing flow, that banner is blocked before render and logged with `banner_hidden_reason.reason = "unresolvable_route"`.
- Required Firestore index for the first rollout: `bannerSources` collection query on `moderationStatus ASC`, `surfaceAllowlist ARRAY_CONTAINS`.
