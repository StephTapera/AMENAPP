...
export interface RetrievedChunk {
  chunkId: string;
  source: "scripture" | "theology" | "church" | "userData" | "platform";
  content: string;
  citation: string;
...
export async function retrieveScripture(
  query: string,
  translations: string[] = ["BSB", "WEB", "KJV"],
  ...
  for (const translation of translations) {
    if (chunks.length >= SCRIPTURE_MAX) break;
    ...
    try {
      ...
      const verses: Array<{
        ...
        bookId?: string;
        chapterId?: string;
      }> = response.data?.data?.verses ?? response.data?.data?.passages ?? [];
      ...
      for (const verse of verses) {
        ...
        const rawText: string = typeof verse.text === "string"
      ...
      console.warn(
        `retrieveScripture: error fetching ${translation} —`,
 ...
 */
export async function retrieveTheology(
  query: string,
  ...
  try {
    ...
    if (snap.empty) return chunks;

    const words = queryWords(query);

    const scoredDocs: Array<{ score: number; doc: FirebaseFirestore.QueryDocumentSnapshot }> = [];
    ...
    console.warn(
      "retrieveTheology: Firestore error —",
      err instanceof Error ? err.message : String(err),
    );
  }
 ...
 */
export async function retrieveChurchData(
  query: string,
  userId: string,
  db: FirebaseFirestore.Firestore,
): Promise<RetrievedChunk[]> {
  ...
  try {
    ...
    console.warn(
      "retrieveChurchData: Firestore error —",
      err instanceof Error ? err.message : String(err),
    );
  }
...
export async function retrieveUserData(
  ...
  try {
    ...
    console.warn(
      "retrieveUserData/bereanNotes: Firestore error —",
      err instanceof Error ? err.message : String(err),
    );
  }
  ...
  try {
    ...
    console.warn(
      "retrieveUserData/savedVerses: Firestore error —",
      err instanceof Error ? err.message : String(err),
    );
  }
...
export async function retrieveEvidence(
  ...
  try {
    ...
    console.warn(
      "retrieveEvidence: could not read feature flag —",
      err instanceof Error ? err.message : String(err),
    );
  }
  ...
  const retrievalPromises: Promise<RetrievedChunk[]>[] = [];

  const wantsBible =
    intentsLower.some((i) => i.includes("bible") || i === "scripture");

  const wantsChurch =
    intentsLower.some((i) => i.includes("church") || i === "church");

  const wantsTheology =
    intentsLower.some(
      (i) => i.includes("theolog") || i === "theology" || i === "doctrine",
    );
  ...
  for (const result of results) {
    if (result.status === "fulfilled") {
      ...
      console.warn(
        "retrieveEvidence: a retriever rejected —",
