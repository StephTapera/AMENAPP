import {
  BILContractError,
  BILCompactionEpisodeDraft,
  BILSourceCardDraft,
  assertServerContentAllowed,
  sanitizeServerEnvelope,
  validateCompactionEpisodeDraft,
  validateContextPackageDraft,
  validateLedgerEntryDraft,
  validateSourceCardDraft,
} from './bilContracts'

function baseEpisode(): BILCompactionEpisodeDraft {
  return {
    tier: 'tier_c',
    sourceKind: 'thread',
    sourceId: 'thread-1',
...
function baseSourceCard(): BILSourceCardDraft {
  return {
    tier: 'tier_c',
...
describe('BIL contract guards', () => {
  ...
  test('sanitizes allowed server envelopes by stripping plaintext', () => {
    const sanitized = sanitizeServerEnvelope({
      tier: 'tier_c',
      sourceKind: 'thread',
      sourceId: 'thread-1',
  ...
  test('validates compaction episode turn ranges and summary arrays', () => {
    ...
    expect(() => validateCompactionEpisodeDraft(episode)).not.toThrow()
