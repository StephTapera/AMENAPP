...
exports.notifyCommunityLinkInvite = void 0;
...
// MARK: - Trigger
exports.notifyCommunityLinkInvite = (0, firestore_1.onDocumentCreated)("amenCommunities/{communityId}/links/{linkId}", async (event) => {
    ...
    const fromCommunityId = linkData?.fromCommunityId ?? "";
    const toCommunityId = linkData?.toCommunityId ?? "";
    const spaceId = linkData?.spaceId ?? "";
    const scope = linkData?.scope ?? "";
    ...
    if (targetAdminUids.length === 0) {
        logger.info(`[notifyCommunityLinkInvite] No admins found for community=${toCommunityId}. No notifications sent.`);
        return null;
    }
    ...
    logger.info(`[notifyCommunityLinkInvite] Sent ${totalSent} push(es) to ${targetAdminUids.length} admin(s) ` +
...
async function sendPushToUser(userId, title, body, data) {
    try {
        ...
        const deviceSnap = await db
            ...
            .get();
        if (!deviceSnap.empty) {
            tokens = deviceSnap.docs.map((d) => d.data().token).filter(Boolean);
        }
        ...
        if (staleTokens.length > 0) {
            ...
            await batch.commit();
            logger.info(`[notifyCommunityLinkInvite] Removed ${staleTokens.length} stale token(s) for ${userId}.`);
        }
        return sentCount;
    }
    catch (e) {
