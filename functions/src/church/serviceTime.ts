...
import type { ServiceTime } from "../contracts/church";
...
function zonedNow(nowMs: number, timezone: string): { dow: number; minutesOfDay: number } {
...
function parseStartLocal(startLocal: string): number | null {
...
function minutesUntil(service: ServiceTime, nowMs: number): number | null {
...
export function computeNextService(serviceTimes: ServiceTime[], nowMs: number): NextService | null {
...
export function isOpenNow(serviceTimes: ServiceTime[], nowMs: number): boolean {
  for (const s of serviceTimes) {
    ...
    if (tz.dow === s.dayOfWeek) {
      ...
      if (tz.minutesOfDay >= start && tz.minutesOfDay < end) return true;
    }
  }
  return false;
}
