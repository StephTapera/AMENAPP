...
import { ScriptureDetection } from "../types";

// ── Book registry ─────────────────────────────────────────────────────────────
// Maps lowercase full names and abbreviations to OSIS book codes.
// Numbered books use the "1 " / "2 " prefix form.

type BookEntry = { osis: string; fullName: string };

// prettier-ignore
const BOOK_ENTRIES: Array<{ names: string[]; osis: string; fullName: string }> = [
  // Old Testament
...
const BOOK_LOOKUP = new Map<string, BookEntry>();
 ...
 */
export function detectReferences(text: string, blockId: string): ScriptureDetection[] {
  const results: ScriptureDetection[] = [];
...
export function parseRefs(text: string): ParsedRef[] {
  ...
  // Build a combined pattern. We'll iterate through the text using lastIndex
  // and try to match book names at each word boundary.

  // For simplicity, we scan with a broad token regex and validate each match.
  // Pattern: optional (1|2|3 space?) then word chars, then optionally more words

  // We'll build an array of candidate positions: { bookKey, osisCode, fullName, start, end }
  interface Candidate {
    bookEntry: BookEntry;
    rawBook: string; // matched text for display
    start: number;
    end: number; // end of the book token (exclusive)
  }
  ...
  while ((m = singleBookRegex.exec(text)) !== null) {
    ...
    const overlaps = candidates.some(
      (c) => matchStart < c.end && matchEnd > c.start
    );
  ...
  for (const cand of candidates) {
    ...
    if (startVerse !== undefined && startVerse < 1) continue;
    if (rangeEnd !== undefined && startVerse !== undefined && rangeEnd < startVerse && rangeEndChapter === undefined) continue;
    ...
    const { osisRef, display } = buildOsisRef(
      cand.bookEntry,
      chapter,
      startVerse,
      rangeEnd,
 ...
 */
export function detectReferencesInBlocks(
  blocks: Array<{ blockId: string; text: string }>
): ScriptureDetection[] {
  const all: ScriptureDetection[] = [];
