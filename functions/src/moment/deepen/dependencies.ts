...
import {
  ...
  MomentSaveInput,
} from "../../berean/momentAdapter";

const firestore = getFirestore();

export function createMomentDeepenDependencies(): MomentDeepenDependencies {
  return {
    berean: {
      run: runBereanDeepen,
    },
...
async function runBereanDeepen(input: BereanDeepenInput): Promise<BereanDeepenDraft> {
...
async function reviewConstitutionalIntelligence(
...
async function reviewGuardianAegis(input: GuardianAegisInput) {
...
async function crossReferenceLivingMemory(query: LivingMemoryQuery): Promise<LivingMemoryHit[]> {
...
async function saveMomentOutput(input: MomentSaveInput): Promise<void> {
...
function formationSummary(refId: string): string {
...
function citationList(memory: LivingMemoryHit[], fallback: string): string[] {
...
function scoreMemoryHit(needle: string, content: string): number {
