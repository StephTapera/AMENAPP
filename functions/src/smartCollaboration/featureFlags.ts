...
export type SmartCollabFlagKey =
  (typeof SmartCollabFlags)[keyof typeof SmartCollabFlags];
 ...
 */
export async function isSmartFlagEnabled(
  flagKey: SmartCollabFlagKey
): Promise<boolean> {
  try {
    ...
    const raw = (defaultValue as { value: string }).value;
    return raw === "true";
  } catch (err) {
    // Remote Config fetch failed — safe default: disabled.
    console.warn(
      `[SmartCollab] Remote Config fetch failed for flag "${flagKey}". Defaulting to false.`,
      err
    );
    return false;
  }
