...
import type { SmartCallableRequest, SmartCallableResponse } from "./contracts";
import {
  SmartPaths,
  type ThreadSummary,
  type ThreadSmartContext,
  type ThreadDetectedAction,
  type ThreadPrayerSignal,
  type GroupPulse,
} from "./contracts";
import { withSecureSmartCallable } from "./secureCallable";
import { SmartCollabFlags, isSmartFlagEnabled } from "./featureFlags";
import { labelAsSuggested, sanitizeForLogging } from "./safety";
...
// MARK: - Moderation Gate (inline — does not use conversationOS engine to avoid circular dep)
// Implements the same crisis/harmful/divine-authority checks without importing
// from a sibling module outside smartCollaboration/.
...
function moderateText(text: string): ModerationResult {
  ...
  // Crisis detection
  if (
    /\b(suicide|self.harm|kill myself|end my life|harm myself|danger to (self|others))\b/.test(
      lower
    )
  ) {
  ...
  if (
    /god (is telling|told|commanded|wants) (this|your|the) group/.test(lower) ||
...
// FirebaseFirestore namespace import for Firestore type references
...
async function fetchMessages(
  ...
  return snap.docs.map((doc) => ({
    id: doc.id,
    timestamp: (doc.data().timestamp as FirebaseFirestore.Timestamp) ?? null,
    senderId: (doc.data().senderId as string) ?? "",
    body: doc.data().body as string | undefined,
  }));
}
...
async function fetchSingleMessage(
  ...
  return {
    id: snap.id,
    timestamp: (snap.data()?.timestamp as FirebaseFirestore.Timestamp) ?? null,
    senderId: (snap.data()?.senderId as string) ?? "",
    body: snap.data()?.body as string | undefined,
  };
}
...
function sanitizeMessagesForLog(
  messages: MessageDoc[]
): Array<{ messageId: string; timestamp: FirebaseFirestore.Timestamp | null }> {
...
function buildMessageBlock(messages: MessageDoc[]): string {
...
export const generateThreadSummary = functions.https.onCall(
  { enforceAppCheck: true, secrets: [CLAUDE_KEY] },
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: SmartCallableRequest, db) => {
        // 1. Feature flag gate — FIRST, before any Firestore reads
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.SMART_CONTEXT);
        if (!enabled) {
          return { success: false, error: "feature_disabled" } as SmartCallableResponse;
        }

        // 2. Read last 50 messages — log only ids/timestamps, never body text
        const messages = await fetchMessages(db, data, 50);
        if (messages.length === 0) {
          return { success: true } as SmartCallableResponse;
        }

        console.info(
          "[SmartCollab:generateThreadSummary] messages fetched",
          sanitizeForLogging({
...
{"summary": "<string>", "keyThemes": ["<string>", ...], "lastSourceMessageId": "${lastSourceMessageId}"}`;
        ...
        const systemPrompt =
          "You are a helpful assistant for a faith-based social platform. " +
        ...
        try {
          ...
          console.error(
            "[SmartCollab:generateThreadSummary] LLM call failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          throw new functions.https.HttpsError("internal", "Summary generation failed.");
        }

        // 5. Parse LLM JSON response
        let parsed: { summary: string; keyThemes: string[]; lastSourceMessageId: string };
        try {
          ...
          console.error(
            "[SmartCollab:generateThreadSummary] JSON parse failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          throw new functions.https.HttpsError("internal", "Summary parsing failed.");
        }
        ...
        if (!modResult.passed) {
          console.warn(
            "[SmartCollab:generateThreadSummary] moderation rejected",
            sanitizeForLogging({ threadId: data.threadId, reason: modResult.reason ?? "unknown" })
          );
          return { success: true } as SmartCallableResponse; // silent fail, not an error
        }
        ...
        const oldestTimestamp =
          ...
          (FieldValue.serverTimestamp() as unknown as FirebaseFirestore.Timestamp);
        ...
        const summaryDocId = uuidv4();
        const summaryPayload: Omit<ThreadSummary, "generatedAt" | "messageRangeStart" | "messageRangeEnd"> & {
        ...
        const summaryPath =
          data.threadType === "dm"
            ? SmartPaths.dmSummary(data.threadId)
            : SmartPaths.channelSummary(data.spaceId!, data.channelId!);

        await db.doc(summaryPath).set(summaryPayload);

        // 9. Write ThreadSmartContext
        const smartContextPayload: Omit<ThreadSmartContext, "generatedAt"> & {
          generatedAt: FirebaseFirestore.FieldValue;
        } = {
          id: summaryDocId,
          threadId: data.threadId,
        ...
        const smartContextPath =
          data.threadType === "dm"
            ? SmartPaths.dmSmartContext(data.threadId)
            : SmartPaths.channelSmartContext(data.spaceId!, data.channelId!);

        await db.doc(smartContextPath).set(smartContextPayload);

        console.info(
          "[SmartCollab:generateThreadSummary] complete",
          sanitizeForLogging({ threadId: data.threadId, jobId: summaryDocId })
        );

        return { success: true, jobId: summaryDocId } as SmartCallableResponse;
      }
    )
);

// MARK: - Callable 2: detectPrayerContextForThread

interface PrayerDetectRequest extends SmartCallableRequest {
  messageId: string;
}

export const detectPrayerContextForThread = functions.https.onCall(
  { enforceAppCheck: true, secrets: [CLAUDE_KEY] },
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: PrayerDetectRequest, db) => {
        // 1. Feature flag gate
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.PRAYER_DETECTION);
        if (!enabled) {
          return { success: false, error: "feature_disabled" } as SmartCallableResponse;
        }
        ...
        if (!data.messageId) {
          throw new functions.https.HttpsError(
            "invalid-argument",
            "messageId is required for prayer detection."
          );
        }
        ...
        console.info(
          "[SmartCollab:detectPrayerContext] analyzing message",
          sanitizeForLogging({ threadId: data.threadId, messageId: message.id })
        );
...
If not a prayer request, set confidence to 0 and themeCategory to "other".`;
        ...
        const systemPrompt =
          "You classify whether a message is a prayer request. " +
        ...
        try {
          ...
          console.error(
            "[SmartCollab:detectPrayerContext] LLM call failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
        ...
        let parsed: { isPrayerRequest: boolean; confidence: number; themeCategory: string };
        try {
          const cleaned = rawOutput.replace(/^```json?\n?/i, "").replace(/\n?```$/i, "").trim();
          parsed = JSON.parse(cleaned) as typeof parsed;
        } catch {
          return { success: true } as SmartCallableResponse; // silent fail
        }
        ...
        if (!modResult.passed) {
          console.warn(
            "[SmartCollab:detectPrayerContext] moderation rejected",
            sanitizeForLogging({ threadId: data.threadId, reason: modResult.reason ?? "unknown" })
          );
          return { success: true } as SmartCallableResponse;
        }
        ...
        const signalPayload: Omit<ThreadPrayerSignal, "generatedAt"> & {
          generatedAt: FirebaseFirestore.FieldValue;
        } = {
          id: signalId,
          threadId: data.threadId,
        ...
        const signalsPath =
          data.threadType === "dm"
            ? SmartPaths.dmPrayerSignals(data.threadId)
            : SmartPaths.channelPrayerSignals(data.spaceId!, data.channelId!);

        await db.collection(signalsPath).doc(signalId).set(signalPayload);

        console.info(
          "[SmartCollab:detectPrayerContext] signal stored",
          sanitizeForLogging({ threadId: data.threadId, signalId })
        );
...
export const extractThreadActions = functions.https.onCall(
  { enforceAppCheck: true, secrets: [CLAUDE_KEY] },
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: SmartCallableRequest, db) => {
        // 1. Feature flag gate
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.ACTION_EXTRACTION);
        if (!enabled) {
        ...
        console.info(
          "[SmartCollab:extractThreadActions] messages fetched",
          sanitizeForLogging({ threadId: data.threadId, messageCount: messages.length })
        );
...
Return valid JSON only, no prose outside the JSON array.`;
        ...
        const systemPrompt =
          "You extract action items from conversation threads. " +
        ...
        try {
          ...
          console.error(
            "[SmartCollab:extractThreadActions] LLM call failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          throw new functions.https.HttpsError("internal", "Action extraction failed.");
        }

        // 4. Parse response
        let rawActions: Array<{
          actionType: ThreadDetectedAction["actionType"];
        ...
        try {
          const cleaned = rawOutput.replace(/^```json?\n?/i, "").replace(/\n?```$/i, "").trim();
          rawActions = JSON.parse(cleaned) as typeof rawActions;
          if (!Array.isArray(rawActions)) rawActions = [];
        } catch {
          console.error(
            "[SmartCollab:extractThreadActions] JSON parse failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          return { success: true, count: 0 };
        }
        ...
        const validActionTypes: ThreadDetectedAction["actionType"][] = [
          "followUp",
          "decision",
          "commitment",
          "openQuestion",
        ...
        const actionsPath =
          data.threadType === "dm"
            ? SmartPaths.dmSmartActions(data.threadId)
            : SmartPaths.channelSmartActions(data.spaceId!, data.channelId!);
        ...
        for (const action of filtered) {
          ...
          const actionPayload: Omit<ThreadDetectedAction, "generatedAt"> & {
            generatedAt: FirebaseFirestore.FieldValue;
          } = {
            id: actionId,
            threadId: data.threadId,
            actionType: action.actionType,
            suggestedText: labelAsSuggested(action.suggestedText), // Rule #6
        ...
        console.info(
          "[SmartCollab:extractThreadActions] actions written",
          sanitizeForLogging({ threadId: data.threadId, count })
        );
...
export const generateGroupPulse = functions.https.onCall(
  { enforceAppCheck: true, secrets: [CLAUDE_KEY] },
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: SmartCallableRequest, db) => {
        // 1. Feature flag gate
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.GROUP_PULSE);
        if (!enabled) {
          return { success: false, error: "feature_disabled" } as SmartCallableResponse;
        }
        ...
        if (data.threadType !== "channel") {
          throw new functions.https.HttpsError(
            "invalid-argument",
            "generateGroupPulse requires threadType: channel."
          );
        }
        ...
        console.info(
          "[SmartCollab:generateGroupPulse] messages fetched",
          sanitizeForLogging({
            threadId: data.threadId,
            messageCount: messages.length,
...
{"urgency": "<string>", "topicMomentum": <number>, "isAligned": <true|false|null>, "alignmentEvidenceMessageIds": [<string>, ...]}`;
        ...
        const systemPrompt =
          "You assess group conversation dynamics. " +
        ...
        try {
          ...
          console.error(
            "[SmartCollab:generateGroupPulse] LLM call failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          throw new functions.https.HttpsError("internal", "Group pulse generation failed.");
        }
        ...
        try {
          ...
          console.error(
            "[SmartCollab:generateGroupPulse] JSON parse failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          throw new functions.https.HttpsError("internal", "Group pulse parsing failed.");
        }

        // 5. Validate and clamp urgency/momentum values
        const validUrgency: GroupPulse["urgency"][] = ["normal", "elevated", "urgent"];
        const urgency: GroupPulse["urgency"] = validUrgency.includes(
          parsed.urgency as GroupPulse["urgency"]
        )
          ? (parsed.urgency as GroupPulse["urgency"])
          : "normal";
        ...
        const isAligned: boolean | undefined =
          parsed.isAligned === true || parsed.isAligned === false
            ? parsed.isAligned
            : undefined;

        const alignmentEvidenceMessageIds: string[] = Array.isArray(
          parsed.alignmentEvidenceMessageIds
        )
        ...
        const pulsePayload: Omit<GroupPulse, "generatedAt"> & {
          generatedAt: FirebaseFirestore.FieldValue;
        } = {
          id: pulseId,
        ...
        const pulsePath = SmartPaths.channelPulse(data.spaceId!, data.channelId!);
        await db.doc(pulsePath).set(pulsePayload);

        console.info(
          "[SmartCollab:generateGroupPulse] pulse written",
          sanitizeForLogging({ threadId: data.threadId, pulseId })
        );

        return { success: true, jobId: pulseId } as SmartCallableResponse;
      }
    )
);

// MARK: - Callable 5: generateSmartReplies

interface SmartRepliesRequest extends SmartCallableRequest {
  lastMessageId: string;
}

interface SmartRepliesResponse extends SmartCallableResponse {
  replies: string[];
}
...
export const generateSmartReplies = functions.https.onCall(
  ...
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: SmartRepliesRequest, db) => {
        // 1. Feature flag gate
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.SMART_REPLIES);
        if (!enabled) {
          return { success: false, error: "feature_disabled", replies: [] } as SmartRepliesResponse;
        }

        // 2. Read last 5 messages for context
        const messages = await fetchMessages(db, data, 5);
        if (messages.length === 0) {
          return { success: true, replies: [] } as SmartRepliesResponse;
        }

        console.info(
          "[SmartCollab:generateSmartReplies] messages fetched",
          sanitizeForLogging({ threadId: data.threadId, messageCount: messages.length })
        );
...
{"replies": ["<string>", "<string>", "<string>"]}`;
        ...
        const systemPrompt =
          "You suggest short, faith-affirming reply suggestions for a messaging app. " +
        ...
        try {
          ...
          console.error(
            "[SmartCollab:generateSmartReplies] LLM call failed",
            sanitizeForLogging({ threadId: data.threadId })
          );
          return { success: true, replies: [] } as SmartRepliesResponse;
        }
        ...
        for (const reply of parsed.replies.slice(0, 3)) {
          ...
          if (modResult.passed) {
            ...
            safeReplies.push(labelAsSuggested(reply));
          }
        }

        // 5. Do NOT persist to Firestore — ephemeral, returned directly
        console.info(
          "[SmartCollab:generateSmartReplies] replies generated",
          sanitizeForLogging({ threadId: data.threadId, count: safeReplies.length })
        );

        return { success: true, replies: safeReplies } as SmartRepliesResponse;
      }
    )
);

// MARK: - Callable 6: requestMediaTranscription

interface MediaTranscriptionRequest extends SmartCallableRequest {
  mediaMessageId: string;
  mediaType: "audio" | "video";
}

interface MediaTranscriptionResponse extends SmartCallableResponse {
  jobId: string;
}

export const requestMediaTranscription = functions.https.onCall(
  { enforceAppCheck: true },
  async (request) =>
    withSecureSmartCallable(
      request,
      async (uid, data: MediaTranscriptionRequest, db) => {
        // 1. Feature flag gate
        const enabled = await isSmartFlagEnabled(SmartCollabFlags.MEDIA_INTELLIGENCE);
        if (!enabled) {
          return { success: false, error: "feature_disabled", jobId: "" } as MediaTranscriptionResponse;
        }
        ...
        if (!message) {
          throw new functions.https.HttpsError(
            "not-found",
            "Media message not found in this thread."
          );
        }
        ...
        let mediaURL: string;
        {
          ...
          const snap = await docRef.get();
          if (!snap.exists) {
            throw new functions.https.HttpsError("not-found", "Media message not found.");
          }
          mediaURL = (snap.data()?.mediaURL as string) ?? "";
          if (!mediaURL) {
            throw new functions.https.HttpsError(
              "failed-precondition",
              "Media message has no mediaURL."
            );
          }
        }
        ...
        if (data.spaceId) jobPayload["spaceId"] = data.spaceId;
        if (data.channelId) jobPayload["channelId"] = data.channelId;

        await db.collection("mediaJobs").doc(jobId).set(jobPayload);

        console.info(
          "[SmartCollab:requestMediaTranscription] job queued",
          sanitizeForLogging({
            threadId: data.threadId,
            jobId,
            mediaType: data.mediaType,
        ...
        return { success: true, jobId } as MediaTranscriptionResponse;
      }
    )
);
