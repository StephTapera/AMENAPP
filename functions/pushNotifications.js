...
async function checkNotificationPreference(userId, notificationType) {
  try {
    ...
    if (!userDoc.exists) {
      console.log(`⚠️ User ${userId} not found, assuming notifications enabled`);
    ...
    if (!settingsKey) {
      console.log(`⚠️ Unknown notification type: ${notificationType}, assuming enabled`);
      return true;
    }
...
async function sendPushNotificationToUser(userId, title, body, data = {}, notificationType = null) {
  try {
    ...
    if (notificationType) {
      ...
      if (!isEnabled) {
        console.log(`🔕 Notification skipped (user preference): ${notificationType} for ${userId}`);
    ...
    async (event) => {
      ...
      try {
        ...
        if (!followerDoc.exists) {
          console.log("⚠️ Follower user not found");
          return null;
        }

        const followerData = followerDoc.data();
        // ✅ FIX: Multiple fallbacks to prevent "undefined" in notifications
        ...
        // ✅ IDEMPOTENCY: Use deterministic ID (follow_actorId_userId)
        ...
        console.log(`✅ Follow notification created for ${followingId} from ${followerName}`);

        // ✅ NEW: Check user preference before sending push
...
exports.onUserUnfollow = onDocumentDeleted(
    ...
    async (event) => {
      ...
      try {
        // ✅ IDEMPOTENCY: Use the same deterministic ID
        const notificationId = `follow_${followerId}_${followingId}`;
...
exports.onCommentReply = onDocumentCreated(
    ...
    async (event) => {
      ...
      try {
        ...
        // ✅ FIX: Use deterministic ID to prevent reply spam
        // Same person replying multiple times = one notification (like comments)
        ...
        await sendPushNotificationToUser(
            ...
            "reply", // Notification type for preference check
        );
      ...
      try {
        ...
        if (existingNotif.exists) {
          ...
          if (!userExists) {
            ...
            console.log(`✅ Updated grouped amen notification (${actors.length} users)`);
          }
        } else {
          // Create new grouped notification
          const notification = {
          ...
          await notificationRef.set(notification);
          console.log(`✅ Created new grouped amen notification`);
        }
      ...
      try {
        ...
        if (existingNotif.exists) {
          ...
          if (updatedActors.length === 0) {
            ...
            console.log(`✅ Deleted grouped amen notification (no more actors)`);
          } else {
            ...
            console.log(`✅ Updated grouped amen notification (${updatedActors.length} users remaining)`);
          }
        }

        console.log(`✅ Processed amen deletion for ${postData.authorId}`);
...
exports.onMessageReaction = onDocumentCreated(
    ...
    async (event) => {
      ...
      console.log(`❤️ New reaction on message ${messageId} in conversation ${conversationId}`);
      ...
      try {
        ...
        if (messageSenderId === reactorId) {
          console.log("⏭️ Skipping - user reacted to their own message");
          return null;
        }
        ...
        console.log(`✅ Message reaction notification created for ${messageSenderId}`);
        ...
        console.error("❌ Error in onMessageReaction:", error);
        return null;
      }
    },
);
