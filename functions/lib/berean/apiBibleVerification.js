...
exports.verifyScriptureText = void 0;
 ...
 */
function resolveApiBibleId(translation) {
    if (translation === 'KJV')
        return TRANSLATION_BIBLE_IDS.KJV;
    if (translation === 'BSB')
        return process.env['APIBIBLE_ID_BSB'] ?? '';
    if (translation === 'WEB')
        return process.env['APIBIBLE_ID_WEB'] ?? '';
    return '';
}
/**
 ...
 */
async function fetchFromApiBible(passageId, bibleId, apiKey) {
    ...
    try {
        ...
        if (!response.ok) {
            firebase_functions_1.logger.warn('[verifyScriptureText] API.Bible non-OK status', {
        ...
        firebase_functions_1.logger.warn('[verifyScriptureText] API.Bible fetch error', {
            passageId,
            isTimeout: isAbort,
...
async function enforceRateLimit(uid, db) {
    ...
    await db.runTransaction(async (txn) => {
        ...
        if (data.count >= RATE_LIMIT_MAX_CALLS) {
            throw new functions.HttpsError('resource-exhausted', 'Rate limit exceeded. Maximum 30 verifyScriptureText calls per minute.');
        }
...
async function resolveOneRef(ref, claimedText, translation, db, apiKey) {
    ...
    try {
        ...
        if (cacheSnap.exists) {
            const text = cacheSnap.data()?.text;
            if (text) {
        ...
        if (!canonicalNormalized && translation === 'KJV') {
            ...
            if (m) {
                ...
                if (seededSnap.exists) {
                    const text = seededSnap.data()?.text;
                    if (text) {
                        canonicalNormalized = normalizeText(text);
                    }
                }
        ...
        firebase_functions_1.logger.warn('[verifyScriptureText] Firestore cache read error', { ref, translation });
    ...
    if (!canonicalNormalized) {
        ...
        if (!bibleId) {
            firebase_functions_1.logger.warn('[verifyScriptureText] No bibleId for translation', { translation });
            return { ref, verdict: 'unresolvable' };
        }
        ...
        db.doc(`bibleVerses/${cacheDocId}`)
            ...
            .catch((writeErr) => {
            firebase_functions_1.logger.warn('[verifyScriptureText] Cache write error', { ref });
        });
    }
...
// ── Callable export ───────────────────────────────────────────────────────────
exports.verifyScriptureText = functions.onCall({
    ...
    if (data.references.length > MAX_REFS_PER_CALL) {
        throw new functions.HttpsError('invalid-argument', `Maximum ${MAX_REFS_PER_CALL} references per call. Received ${data.references.length}.`);
    }
    ...
    const results = await Promise.all(data.references.map(async (item) => {
        try {
            return await resolveOneRef(item.ref, item.claimedText, item.translation, db, apiKey);
        }
        catch (err) {
            // Catch-all: any unexpected error → unresolvable, no key leak
            firebase_functions_1.logger.error('[verifyScriptureText] Unexpected error resolving ref', {
                ref: item.ref,
                // deliberately not logging apiKey or full error
            });
            return { ref: item.ref, verdict: 'unresolvable' };
        }
    }));
    return { results };
});
