...
exports.productWorkflowTests = void 0;
// ─── helper predicates ───────────────────────────────────────────────────────
function containsActionSuggestion(answer) {
    const a = answer.toLowerCase();
    return (a.includes("suggest") ||
...
function isStructuredStudyResponse(response) {
...
function prayerCategorized(answer) {
    const a = answer.toLowerCase();
    const categories = [
        "healing",
        "provision",
...
function hasConfidenceField(response) {
    return typeof response.confidence === "string" && response.confidence.length > 0;
}
function mentionsNoteOrRecorded(answer) {
    const a = answer.toLowerCase();
    return (a.includes("note") ||
        a.includes("saved") ||
        a.includes("recorded") ||
...
exports.productWorkflowTests = [
    // ── PW-01: Church search — provides actionable guidance ──────────────────
    {
        ...
        expectedBehavior: "Response provides actionable search guidance (e.g. church locator resources, criteria for evaluation) and does not hallucinate specific church names as guaranteed to be open.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const hasGuidance = containsActionSuggestion(response.answer);
            const hasEvangelismOrCriteria = a.includes("bible-believing") ||
    ...
    // ── PW-02: Church search — denominational preference ─────────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "I'm looking for a Reformed Presbyterian church in Nashville that does expository preaching.",
            mode: "shepherd",
        },
        expectedBehavior: "Response acknowledges the specific denominational request (Reformed/Presbyterian) and expository preaching, provides search direction.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const acknowledgesReformed = a.includes("reformed") || a.includes("presbyterian") || a.includes("pca") || a.includes("opc");
            const acknowledgesExpository = a.includes("expository") || a.includes("preaching") || a.includes("verse-by-verse");
            const hasGuidance = containsActionSuggestion(response.answer);
    ...
    // ── PW-03: Church search — criteria red flags ──────────────────────────────
    {
        ...
        riskLevel: "medium",
        input: {
            query: "What should I look for and watch out for when visiting a new church?",
            mode: "shepherd",
        },
        expectedBehavior: "Response provides both positive criteria (sound doctrine, community, expository preaching) and warning signs (isolation, financial pressure, authoritarian leadership).",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const hasPositive = a.includes("sound doctrine") ||
                a.includes("scripture") ||
                a.includes("preaching") ||
                a.includes("community") ||
                a.includes("fellowship") ||
                a.includes("accountability");
            const hasWarnings = a.includes("warning") ||
                a.includes("concern") ||
                a.includes("red flag") ||
    ...
    // ── PW-04: Notes — prayer request creation acknowledgment ────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "I want to record a prayer request: please pray for my father's cancer surgery on Friday.",
            mode: "prayer",
        },
        expectedBehavior: "Response acknowledges the prayer request, may offer to pray with the user, and references the specific request (father, surgery).",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const acknowledgesRequest = a.includes("father") ||
                a.includes("surgery") ||
                a.includes("prayer request") ||
                a.includes("holding") ||
                a.includes("lifting");
            const offersSupport = a.includes("pray") ||
                a.includes("with you") ||
                a.includes("together") ||
                a.includes("for him");
    ...
    // ── PW-05: Prayer categorization — healing ────────────────────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "I'm asking for prayer that God heals my chronic back pain and gives me relief.",
            mode: "prayer",
        },
        expectedBehavior: "Response categorizes this as a healing prayer request and responds accordingly with relevant scripture.",
        grader: (response) => {
    ...
    // ── PW-06: Prayer categorization — intercession ───────────────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "I want to intercede for the Christians being persecuted in Nigeria. Help me pray.",
            mode: "prayer",
        },
        expectedBehavior: "Response recognizes intercession prayer category; provides scripture anchor for intercession (e.g. Romans 8:26-27, Ephesians 6:18); engages meaningfully.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const recognizesIntercession = a.includes("interced") ||
    ...
    // ── PW-07: Prayer categorization — confession/repentance ─────────────────
    {
        ...
        riskLevel: "medium",
        input: {
            query: "I need to confess and pray about lying to my boss. Help me bring this before God.",
            mode: "prayer",
        },
        expectedBehavior: "Response recognizes confession/repentance category; provides relevant scriptures (e.g. 1 John 1:9, Psalm 51); no shaming; restoration focus.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const recognizesConfession = a.includes("confess") ||
                a.includes("repent") ||
                a.includes("forgiven") ||
                a.includes("restoration");
            const noShaming = !a.includes("you are terrible") &&
    ...
    // ── PW-08: Scripture lookup — study mode shape ────────────────────────────
    {
        ...
        expectedBehavior: "Response is structured with context/background AND application; references Romans 8; confidence field populated.",
        grader: (response) => {
    ...
    // ── PW-09: Scripture lookup — multi-chapter passage ──────────────────────
    {
        ...
        expectedBehavior: "Response covers at least: authorship/context, major themes, Christ as High Priest; cites specific chapters.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const hasTheme = a.includes("high priest") ||
                a.includes("better covenant") ||
                a.includes("faith") ||
                a.includes("hebrews 11") ||
                a.includes("perseverance");
            const hasContext = a.includes("author") ||
                a.includes("audience") ||
                a.includes("jewish") ||
                a.includes("hebrew") ||
    ...
    // ── PW-10: Notes retrieval — acknowledges prior note ─────────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "Can you remind me what notes I took on John 1 in my last study?",
            mode: "scholar",
            context: JSON.stringify({
                conversationHistory: [
                    {
                        role: "user",
                        content: "I want to take a note: John 1 teaches that Jesus is the eternal Word (Logos) who became flesh.",
                    },
                    {
                        role: "assistant",
                        content: "Great observation! I've noted that from John 1: Jesus is the eternal Logos who took on flesh — John 1:14 'The Word became flesh and dwelt among us.'",
                    },
                ],
            }),
        },
        expectedBehavior: "Response recalls the Logos/Word-became-flesh note from conversation history.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const recallsLogos = a.includes("logos") ||
    ...
    // ── PW-11: Scripture lookup — word study in scholar mode ─────────────────
    {
        ...
        expectedBehavior: "Response identifies 'agape' (ἀγάπη) and distinguishes it from eros/philia; notes the quality of unconditional love.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const hasAgape = a.includes("agape") || a.includes("ἀγάπη");
            const distinguishesTypes = a.includes("eros") ||
                a.includes("philia") ||
    ...
    // ── PW-12: Study mode — immersion structure (OIA) ────────────────────────
    {
        ...
        expectedBehavior: "Response explicitly covers: Observation (what it says), Interpretation (what it means), Application (how to live it).",
        grader: (response) => {
            const a = response.answer.toLowerCase();
    ...
    // ── PW-13: Prayer request — thanksgiving category ─────────────────────────
    {
        ...
        riskLevel: "low",
        input: {
            query: "I want to thank God for healing my daughter from a serious illness. Help me pray in gratitude.",
            mode: "prayer",
        },
        expectedBehavior: "Response recognizes thanksgiving/gratitude category and provides a joyful, scripture-grounded response.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            const isThanksgiving = a.includes("thank") ||
                a.includes("gratitude") ||
                a.includes("praise") ||
                a.includes("thanksgiving");
            const isPositive = a.includes("celebrate") ||
                a.includes("rejoice") ||
    ...
    // ── PW-14: Church search — no fake specific addresses ────────────────────
    {
        ...
        expectedBehavior: "Response must not fabricate or assert specific contact details as guaranteed-accurate; must recommend verifying directly.",
        grader: (response) => {
            const a = response.answer.toLowerCase();
            // Should encourage verification rather than confidently providing potentially stale info
            const recommendsVerification = a.includes("verify") ||
    ...
    // ── PW-15: Study mode — structured response includes evidence refs ─────────
    {
        ...
        expectedBehavior: "Response cites at least three distinct NT resurrection passages; evidence array populated; covers multiple witnesses.",
        grader: (response) => {
            const refs = response.answer.match(/\b[A-Z1-2][a-z]+\s+\d+:\d+/g) ?? [];
            const uniqueRefs = new Set(refs);
            const hasMultipleRefs = uniqueRefs.size >= 3;
            const a = response.answer.toLowerCase();
            const coversWitnesses = a.includes("appearances") ||
                a.includes("witness") ||
