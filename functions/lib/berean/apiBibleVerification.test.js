...
function parseRefToPassageId(ref) {
    const BOOK_MAP = {
...
function assertClose(a, b, delta, message) {
    if (Math.abs(a - b) > delta)
        throw new Error(`FAIL: ${message} (${a} vs ${b}, delta=${delta})`);
}
// ── Test runner ───────────────────────────────────────────────────────────────
const PASS = '✓';
const FAIL = '✗';
...
async function testNearExactMatchStillVerified() {
...
// ── T2: Paraphrase (similarity ~0.7) → "mismatch" with canonicalText ──────────
async function testParaphraseMismatch() {
...
async function testMismatchIncludesCanonicalText() {
...
async function testFabricatedBookUnresolvable() {
...
async function testFabricatedChapterUnresolvable() {
...
async function testNonCanonicalBookUnresolvable() {
...
async function testMalformedRefUnresolvable() {
    // Totally malformed
...
// ── T4: API.Bible error → "unresolvable" ─────────────────────────────────────
async function testApiBibleErrorMapsToUnresolvable() {
    ...
    const fetched = null; // simulates API error
...
async function testApiBibleNonOkStatusUnresolvable() {
...
// ── T5: API.Bible timeout → "unresolvable" ────────────────────────────────────
async function testApiBibleTimeoutUnresolvable() {
...
async function testTimeoutConstantIs5Seconds() {
...
async function testOver20RefsThrows() {
    ...
    try {
        if (refs.length > MAX_REFS_PER_CALL) {
            ...
            const err = new Error(`Maximum ${MAX_REFS_PER_CALL} references per call.`);
...
async function testExactly20RefsDoesNotThrow() {
    const MAX_REFS_PER_CALL = 20;
...
async function testNormalizeStripsVerseNumbers() {
...
async function testNormalizeCasefolds() {
    const input = 'For GOD So LOVED The World';
    const result = normalizeText(input);
    assert(result === result.toLowerCase(), 'Result should be all lowercase');
}
async function testNormalizeCollapseWhitespace() {
...
async function testJaccardIdenticalStrings() {
...
async function testJaccardHalfOverlap() {
...
async function testRateLimitConstants() {
    const RATE_LIMIT_MAX_CALLS = 30;
    const RATE_LIMIT_WINDOW_MS = 60000;
    assert(RATE_LIMIT_MAX_CALLS === 30, 'Rate limit must be 30 calls per window');
    assert(RATE_LIMIT_WINDOW_MS === 60000, 'Window must be 60 seconds');
}
// ── Main ──────────────────────────────────────────────────────────────────────
