/**
 * discussionFunctions.js — Discussion system callable Cloud Functions (v2)
 * Migrated from firebase-functions/v1 to v2 for Node.js 22 compatibility.
 *
 * LLM key: BEREAN_LLM_KEY secret (defineSecret) — mock adapter used if absent.
 * Embedding key: EMBEDDING_KEY secret (defineSecret) — zero-vector stub if absent.
...
const bereanLLMKey  = defineSecret("BEREAN_LLM_KEY");
const embeddingKey  = defineSecret("EMBEDDING_KEY");
...
async function discussionRateLimit(db, uid, feature, maxPerHour) {
  ...
  await db.runTransaction(async (t) => {
    ...
    if (count >= maxPerHour) {
      throw new HttpsError("resource-exhausted", `Rate limit reached for ${feature}. Try again later.`);
    }
...
const computeReputation = onCall(
  ...
  async (request) => {
    ...
    const snap = await db.collection("reputationEvents").where("toUID","==",userId).limit(500).get();
    ...
    return { uid: userId, totalPoints: total, badgeTier: badgeTier(total), breakdown };
  }
);
...
// processEmbeddingQueue — Firestore trigger (v2)
const processEmbeddingQueue = onDocumentCreated(
  { document: "embeddingQueue/{docId}", region: "us-central1", secrets: [embeddingKey] },
  async (event) => {
...
const setAccepted = onCall(
  ...
  async (request) => {
    ...
    const commentId  = String(request.data?.commentId  ?? "").trim();
    const threadId   = String(request.data?.threadId   ?? "").trim();
    const isAccepted = request.data?.isAccepted === true;
    ...
    if (threadSnap.data()?.postAuthorUID !== userId) {
      throw new HttpsError("permission-denied", "Only the post author can accept an answer.");
    }
