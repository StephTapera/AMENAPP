...
exports.matchCommunitiesFromContext = void 0;
...
function resolveYouthSafeOnly(uid, minor) {
    ...
    try {
        ...
        if (mod && typeof mod.enforceMinorConstraints === "function") {
            const decision = mod.enforceMinorConstraints(uid, "communityMatching", ageTier);
 ...
 */
async function readOwnerContextTerms(uid) {
    ...
    try {
        ...
        logger.warn("matchCommunitiesFromContext: facet read failed", { uid, error: err.message });
        return [];
    }
    const terms = [];
    for (const doc of snap.docs) {
        ...
        if (!MATCHABLE_CATEGORIES.has(category))
            continue;
        ...
        if (terms.length >= MAX_FACET_TERMS)
            break;
    }
    // De-dupe + bound.
...
function collectValueTerms(value, out) {
    const { kind, payload } = value;
    const pushStr = (s) => {
        if (typeof s === "string" && s.trim())
            out.push(capStr(s, 80));
    };
    const pushArr = (a) => {
        if (Array.isArray(a))
            a.forEach(pushStr);
 ...
 */
async function retrieveCandidates(youthSafeOnly) {
    const db = admin.firestore();
    const now = admin.firestore.Timestamp.now();
    const queries = TYPE_SOURCES.map(async (src) => {
        try {
            let q = db.collection(src.collection).where("isPublic", "==", true);
            ...
            logger.warn("matchCommunitiesFromContext: candidate query failed", {
                type: src.type, error: err.message,
            });
            return [];
        }
    });
...
function templateExplanation(candidate, terms) {
 ...
 */
async function explainMatches(candidates, terms, uid) {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    let callModel = null;
    try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        ...
        if (callModel) {
            ...
            try {
                ...
                logger.warn("matchCommunitiesFromContext: explain failed", {
                    type: candidate.type, error: err.message,
                });
            }
        }
...
// ─── Callable ───────────────────────────────────────────────────────────────────
exports.matchCommunitiesFromContext = (0, https_1.onCall)({
    region: REGION,
