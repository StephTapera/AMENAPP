...
exports.validateNoExcludedContent = validateNoExcludedContent;
...
const MEDIA_URL_RE = /https?:\/\/[^\s"']+\.(?:jpg|jpeg|png|gif|webp|heic|mp4|mov|m4v|webm|mp3|m4a|wav|aac|pdf)\b/i;
const MEDIA_HOST_RE = /https?:\/\/(?:[a-z0-9.\-]*\.)?(?:firebasestorage\.googleapis\.com|storage\.googleapis\.com|cdn[.\-]|youtube\.com|youtu\.be|vimeo\.com|cloudinary\.com|imgur\.com|tiktok\.com|instagram\.com)\b/i;
// vCard contact block.
 ...
 */
function validateNoExcludedContent(payload) {
    const violations = [];
    const seen = new WeakSet();
    walk(payload, "$", violations, seen);
    return { ok: violations.length === 0, violations };
}
function walk(node, path, out, seen) {
...
function scanString(s, path, out) {
    if (EMAIL_RE.test(s)) {
