...
exports.generateIntroduction = void 0;
...
const MAX_DRAFT_CHARS = 1200; // a self-intro, not an essay
...
function wrapAsInertDocument(text) {
    ...
    switch (kind) {
        ...
        case "communicationStyle": {
            ...
            const styles = Array.isArray(p.conversationStyles)
                ? p.conversationStyles.filter((x) => typeof x === "string").slice(0, 4).map(cleanLeaf)
                : [];
        ...
        case "faithJourney": {
            ...
            const books = Array.isArray(p.favoriteBooks)
                ? p.favoriteBooks.filter((x) => typeof x === "string").slice(0, 5).map(cleanLeaf)
                : [];
            if (church)
                parts.push(`church: ${church}`);
            if (study)
                parts.push(`current study: ${study}`);
            if (books.length)
                parts.push(`favorite books: ${books.join(", ")}`);
            return parts.join(" · ");
        }
...
function cleanLeaf(s) {
    const str = capStr(s, FIELD_CAP).trim();
    if (!str || isExcluded(str))
        return "";
    return str;
}
...
// ─── Callable ───────────────────────────────────────────────────────────────────
exports.generateIntroduction = (0, https_1.onCall)({
    region: REGION,
    ...
    const requestedKeys = Array.from(new Set(rawKeys
        .filter((k) => typeof k === "string")
        .map((k) => capStr(k, 120))
        .filter((k) => k.length > 0))).slice(0, MAX_FACET_KEYS);
    ...
    try {
        ...
        storedFacets = snap.docs.map((d) => d.data());
    }
    catch (err) {
        logger.error("generateIntroduction: facet read failed", {
            uid, communityId, error: err.message,
        });
    ...
    for (const facet of storedFacets) {
        ...
        if (!SERVER_READABLE_TIERS.has(tier))
            continue;
        ...
        if (!category || (!label && !summary))
            continue;
        safeFacets.push({ category, label: label || category, summary });
        if (safeFacets.length >= MAX_FACETS_USED)
            break;
    }
