...
exports.extractContextFacets = void 0;
...
const NVIDIA_API_KEY = (0, params_1.defineSecret)("NVIDIA_API_KEY");
const REGION = "us-central1";
// ─── Caps & constants (mirror facetCandidateJSONSchema / ContextSanitizer) ──────
const MAX_TEXT_CHARS = 16000; // matches ContextSanitizer.rawInputCap
...
const VALID_VISIBILITIES = ["private", "friends", "groups", "church", "public"];
...
function capStr(s, cap) {
    const str = typeof s === "string" ? s : "";
    return str.length > cap ? str.slice(0, cap) : str;
}
function capStrArray(a, cap, maxItems) {
    if (!Array.isArray(a))
        return [];
    return a
...
function hardenCandidate(raw) {
    ...
    if (!/^[a-z0-9_.]+$/.test(key))
        return null;
    if (isExcludedString(key) || isExcludedString(label))
        return null;
    ...
    const suggestedVisibility = "private";
    return { category: category, key, label, value, confidence, suggestedVisibility };
}
function hardenValue(raw) {
    ...
    const v = raw;
    const kind = v.kind;
    if (typeof kind !== "string" || !VALID_VALUE_KINDS.includes(kind)) {
        return null;
    }
    ...
    switch (kind) {
        ...
        case "relationshipCategory": {
            ...
            if (typeof cat !== "string" ||
                !VALID_RELATIONSHIP_CATEGORIES.includes(cat)) {
                return null;
            }
            const note = cleanOptional(p.note);
            return {
                kind: "relationshipCategory",
                payload: { category: cat, note },
            };
        }
        default:
            return null;
...
function cleanOptional(s) {
    if (typeof s !== "string")
        return null;
    const capped = capStr(s, FIELD_CAP);
    if (!capped || isExcludedString(capped))
        return null;
    return capped;
}
/**
 ...
 */
function parseCandidatesFromModel(output) {
    let obj = output;
...
// ─── Callable ───────────────────────────────────────────────────────────────────
exports.extractContextFacets = (0, https_1.onCall)({
    region: REGION,
    ...
    const sanitizationPassId = typeof data.sanitizationPassId === "string" ? data.sanitizationPassId : "";
    // INVARIANT 3 — empty receipt can never extract (mirrors C59 fail-closed).
    if (!sanitizationPassId.trim()) {
        throw new https_1.HttpsError("failed-precondition", "A non-empty sanitizationPassId (Aegis C59 receipt) is required.");
    }
    if (!text.trim()) {
    ...
    try {
        ...
        logger.error("extractContextFacets: callModel threw", {
            uid, sourceLabel, error: err.message,
        });
