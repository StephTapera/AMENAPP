...
exports.badgeReconciliation = functions.pubsub
    ...
    while (true) {
        ...
        if (userSnap.empty)
            break;
        await Promise.allSettled(userSnap.docs.map(async (userDoc) => {
            ...
            try {
                ...
                const storedCount = stateDoc.exists
                ...
                functions.logger.warn(`[badgeReconciliation] Failed for user ${userId}:`, err);
            }
        }));
        lastDoc = userSnap.docs[userSnap.docs.length - 1];
        if (userSnap.size < 200)
            break;
    }
    functions.logger.info(`[badgeReconciliation] Done — processed: ${processed}, corrected: ${corrected}`);
});
    ...
    while (true) {
        ...
        await Promise.allSettled(userSnap.docs.map(async (userDoc) => {
            ...
            try {
                ...
                if (stored?.followersCount !== actualFollowers ||
                ...
                functions.logger.warn(`[followCountReconciliation] Failed for user ${userId}:`, err);
            }
        }));
        lastDoc = userSnap.docs[userSnap.docs.length - 1];
        if (userSnap.size < 100)
            break;
    }
    functions.logger.info(`[followCountReconciliation] Done — processed: ${processed}, corrected: ${corrected}`);
});
...
exports.commentCountReconciliation = functions.pubsub
    ...
    const postSnap = await db.collection("posts")
        ...
        .get();
    let corrected = 0;
    await Promise.allSettled(postSnap.docs.map(async (postDoc) => {
        ...
        try {
            ...
            functions.logger.warn(`[commentCountReconciliation] Failed for post ${postId}:`, err);
        }
    }));
    functions.logger.info(`[commentCountReconciliation] Done — checked: ${postSnap.size}, corrected: ${corrected}`);
});
...
exports.staleConversationCleanup = functions.pubsub
    ...
    await batch.commit();
    functions.logger.info(`[staleConversationCleanup] Deleted ${snap.size} empty conversations`);
});
...
exports.rateLimitWindowCleanup = functions.pubsub
    ...
    functions.logger.info(`[rateLimitWindowCleanup] Deleted ${total} expired rate limit counters`);
});
...
exports.staleTokenPruning = functions.pubsub
    ...
    functions.logger.info(`[staleTokenPruning] Pruned ${total} stale device tokens`);
});
...
exports.staleFollowRequestCleanup = functions.pubsub
    ...
    functions.logger.info(`[staleFollowRequestCleanup] Deleted ${total} stale follow requests`);
});
...
exports.usernameChangeCooldownRelease = functions.pubsub
    ...
    functions.logger.info(`[usernameChangeCooldownRelease] Re-enabled username changes for ${total} users`);
});
...
exports.orphanedMediaCleanup = functions.pubsub
    ...
    const groupIdCache = new Map(); // groupId → hasPost
    do {
        ...
        for (const file of files) {
            ...
            if (!hasPost) {
                try {
                    await file.delete();
                    totalDeleted++;
                    functions.logger.info(`[orphanedMediaCleanup] Deleted orphaned file: ${file.name}`);
                }
                catch (err) {
                    functions.logger.warn(`[orphanedMediaCleanup] Failed to delete ${file.name}:`, err);
                }
            }
        }
    } while (pageToken);
    functions.logger.info(`[orphanedMediaCleanup] Deleted ${totalDeleted} orphaned media files`);
});
