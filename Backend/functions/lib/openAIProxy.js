...
exports.openAIProxy = (0, https_1.onCall)({
    ...
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated to use AI features");
    }
    ...
    const { messages, maxTokens = 1000, temperature = 0.7, model: requestedModel = "gpt-4o-mini", } = data;
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
        throw new https_1.HttpsError("invalid-argument", "Messages array is required and must not be empty");
    }
    ...
    if (messages.length > MAX_MESSAGES_COUNT) {
        throw new https_1.HttpsError("invalid-argument", `Messages array exceeds maximum count of ${MAX_MESSAGES_COUNT}.`);
    }
    for (const msg of messages) {
        if (typeof msg.content === "string" && msg.content.length > MAX_MESSAGE_CONTENT_LENGTH) {
            throw new https_1.HttpsError("invalid-argument", `Message content exceeds maximum length of ${MAX_MESSAGE_CONTENT_LENGTH} characters.`);
        }
    }
    ...
    if (!apiKey) {
        ...
        throw new https_1.HttpsError("unavailable", "OpenAI is not configured. Please contact support.");
    }
    try {
        // Call OpenAI API
        ...
        if (!response.ok) {
            ...
            throw new https_1.HttpsError("unavailable", `OpenAI API error: ${response.status}`);
        }
        ...
        throw new https_1.HttpsError("internal", "Failed to process OpenAI request");
    }
});
