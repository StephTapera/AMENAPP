 ...
 */
exports.invalidateServerFlagCache = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
    ...
    if (!tokenClaims.admin) {
        throw new functions.https.HttpsError("permission-denied", "Only admins can invalidate the server flag cache");
    }
    flagsCache = null;
    cacheExpiresAt = 0;
