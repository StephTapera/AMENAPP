...
exports.deliverQuietHoursDigest = functions.pubsub
    ...
    try {
        ...
        if (!pendingSnap.empty) {
            ...
            const byUser = new Map(); // userId → [docId]
            ...
            functions.logger.info(`[deliverQuietHoursDigest] Processing ${pendingSnap.size} queue entries for ${byUser.size} users`);
            const deleteBatch = db.batch();
            let digestsSent = 0;
            let digestsFailed = 0;
            await Promise.all(Array.from(byUser.entries()).map(async ([userId, docIds]) => {
                try {
                    ...
                    functions.logger.error(`[deliverQuietHoursDigest] Failed for user ${userId}`, e);
                    // Mark entries as failed so they are not retried endlessly.
                    for (const docId of docIds) {
                        deleteBatch.update(db.collection("quietHoursDigestQueue").doc(docId), { status: "failed", failedAt: admin.firestore.FieldValue.serverTimestamp() });
                    }
                    digestsFailed++;
                }
            }));
            await deleteBatch.commit();
            functions.logger.info(`[deliverQuietHoursDigest] Done — sent: ${digestsSent}, failed: ${digestsFailed}`);
        }
 ...
 */
async function sendDigestPush(userId, count) {
    const deviceTokens = await (0, helpers_1.getDeviceTokens)(userId);
    ...
    functions.logger.info(`[deliverQuietHoursDigest] Digest push to ${userId}: ` +
        `${response.successCount}/${tokens.length} tokens succeeded`);
}
//# sourceMappingURL=deliverQuietHoursDigest.js.map