...
const L = vision_1.protos.google.cloud.vision.v1.Likelihood;
function isAtLeast(value, threshold) {
    ...
    const vIdx = order.indexOf(value);
...
function extractOwnerFromPath(filePath) {
    const parts = filePath.split("/");
    return parts.length >= 2 ? parts[1] : null;
}
// ── Write helpers ──────────────────────────────────────────────────────────
async function writeViolationLog(filePath, uploaderUid, violationType, safeSearchResult) {
...
async function writeModerationQueueEntry(filePath, uploaderUid, queueType, priority, safeSearchResult) {
...
async function flagOriginatingDocument(filePath) {
    const parts = filePath.split("/");
...
exports.scanUploadedMedia = functions.storage
    ...
    if (isVideo) {
        ...
        await writeModerationQueueEntry(filePath, uploaderUid, "video_upload_pending_review", "standard", {});
        return;
    }
    ...
    try {
        ...
        await writeModerationQueueEntry(filePath, uploaderUid, "vision_api_failure", "standard", { error: String(err) });
        return;
    }
...
//# sourceMappingURL=mediaScanning.js.map