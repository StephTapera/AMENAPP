...
exports.enforceCommentMentionCap = exports.enforceMessageRestrictions = void 0;
const functions = __importStar(require("firebase-functions")); // kept for helper-level logger usage
 ...
 */
async function hasMessagingFreeze(senderId) {
 ...
 */
async function hasNoContactOrder(senderId, recipientId) {
    const now = admin.firestore.Timestamp.now();
 ...
 */
async function getRecentViolationCounts(senderId, days) {
 ...
 */
async function recordBlockedSendAttempt(senderId, recipientId, conversationId, messageId, blockReason) {
 ...
 */
async function notifySenderOfBlock(senderId, blockReason, restrictionType) {
    const humanReadable = {
 ...
 */
exports.enforceMessageRestrictions = (0, firestore_1.onDocumentCreated)("conversations/{conversationId}/messages/{messageId}", async (event) => {
    ...
    const data = snap.data();
    if (!data) {
        functions.logger.warn(`[AntiHarassment] Message ${messageId} has no data — skipping.`);
        return;
    }
    const senderId = data.senderId ?? "";
    if (!senderId) {
        functions.logger.warn(`[AntiHarassment] Message ${messageId} missing senderId — skipping.`);
        return;
    }
    ...
    const flags = await (0, serverFeatureFlags_1.getServerSafetyFlags)();
    if (!flags.messagingBlockEnforcementEnabled || !flags.antiHarassmentV2Enabled) {
        functions.logger.info(`[AntiHarassment] Enforcement skipped — ` +
    ...
    if (messagingFreeze) {
        functions.logger.info(`[AntiHarassment] Deleting message ${messageId} — sender ${senderId} ` +
        ...
        await recordBlockedSendAttempt(senderId, recipientId, conversationId, messageId, `Active ${messagingFreeze.type} restriction`).catch((err) => functions.logger.error("[AntiHarassment] recordBlockedSendAttempt failed", err));
        return;
    }
    ...
    if (isGroup) {
        ...
        for (const participantId of otherParticipants) {
            ...
            if (noContactGroup) {
                functions.logger.info(`[AntiHarassment] Deleting group message ${messageId} — sender ${senderId} ` +
                ...
                await notifySenderOfBlock(senderId, noContactGroup.reason, NO_CONTACT_TYPE)
                    .catch((err) => functions.logger.error("[AntiHarassment] notifySenderOfBlock failed", err));
                await recordBlockedSendAttempt(senderId, participantId, conversationId, messageId, `Active no-contact order (group chat, participant ${participantId})`).catch((err) => functions.logger.error("[AntiHarassment] recordBlockedSendAttempt failed", err));
                return;
            }
        }
    ...
    if (!recipientId) {
        functions.logger.warn(`[AntiHarassment] Could not determine recipient for conversation ${conversationId}`);
        return;
    }
    ...
    if (noContact) {
        ...
        await notifySenderOfBlock(senderId, noContact.reason, NO_CONTACT_TYPE)
            .catch((err) => functions.logger.error("[AntiHarassment] notifySenderOfBlock failed", err));
        await recordBlockedSendAttempt(senderId, recipientId, conversationId, messageId, "Active no-contact order").catch((err) => functions.logger.error("[AntiHarassment] recordBlockedSendAttempt failed", err));
        return;
    }
    // ── 4. Belt-and-suspenders: check if recipient has blocked sender ─
    const blocked = await isBlockedByRecipient(senderId, recipientId);
    if (blocked) {
        functions.logger.info(`[AntiHarassment] Deleting message ${messageId} — ` +
        ...
        await recordBlockedSendAttempt(senderId, recipientId, conversationId, messageId, "Sender is blocked by recipient").catch((err) => functions.logger.error("[AntiHarassment] recordBlockedSendAttempt failed", err));
        return;
    }
    // ── 5. DM mention cap ─────────────────────────────────────────────
    ...
    if (dmMentionCount > 5) {
        functions.logger.info(`[AntiHarassment] Deleting DM ${messageId} — ${dmMentionCount} mentions exceeds cap of 5.`);
        await snap.ref.delete();
        await notifySenderOfBlock(senderId, "Messages may not contain more than 5 mentions.", "mention_cap").catch(() => { });
        return;
    }
    ...
    try {
        ...
        if (counts.critical >= CRITICAL_VIOLATION_THRESHOLD ||
            counts.severe >= SEVERE_VIOLATION_THRESHOLD) {
            functions.logger.warn(`[AntiHarassment] User ${senderId} has ${counts.critical} critical / ` +
...
exports.enforceCommentMentionCap = (0, firestore_1.onDocumentCreated)("comments/{commentId}", async (event) => {
    ...
    if (mentionCount <= MAX_MENTIONS_PER_COMMENT)
        return;
    const authorId = data.authorId ?? data.userId ?? "";
    v2_1.logger.info(`[CommentMentionCap] Deleting comment ${commentId} — ` +
    ...
    if (authorId) {
        ...
        }).catch((err) => v2_1.logger.error("[CommentMentionCap] enforcementHistory write failed", err));
    }
});
//# sourceMappingURL=antiHarassmentEnforcement.js.map