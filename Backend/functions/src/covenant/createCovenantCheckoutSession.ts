...
export async function createCovenantCheckoutSessionHandler(
...
export const createCovenantCheckoutSession = onCall(
    ...
    async (request) => {
        return createCovenantCheckoutSessionHandler(
            request.auth?.uid,
            (request.data ?? {}) as Partial<CheckoutInput>
        );
    }
);
