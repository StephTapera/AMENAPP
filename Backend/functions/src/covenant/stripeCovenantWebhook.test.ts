...
function makeCheckoutSessionEvent(
    sessionOverrides: Partial<StripeCheckoutSession> = {}
): StripeEvent {
    return {
        id: "evt_test",
        object: "event",
...
describe("writeMemberIndex", () => {
    ...
    test("writes member index with active status on first activation", async () => {
        ...
        const [data, options] = mockDoc.set.mock.calls[0] as [Record<string, unknown>, unknown];
    ...
    test("repeated webhook is idempotent — merge:true prevents data loss", async () => {
        ...
        expect(mockDoc.set).toHaveBeenCalledTimes(1);
        const [data] = mockDoc.set.mock.calls[0] as [Record<string, unknown>];
...
describe("handleStripeEvent", () => {
    ...
    test("customer.subscription.created writes member index", async () => {
        ...
        await handleStripeEvent(
            ...
            makeDb(),
            mockStripeClient as unknown as Pick<StripeClient, "subscriptions">
        );
        expect(mockDoc.set).toHaveBeenCalledTimes(1);
        const [data] = mockDoc.set.mock.calls[0] as [Record<string, unknown>];
        expect(data).toMatchObject({
    ...
    test("unhandled event type — resolves cleanly without writing", async () => {
        const unknownEvent = { type: "payment_intent.succeeded", data: { object: {} } } as StripeEvent;
        await expect(
            handleStripeEvent(unknownEvent, makeDb(), mockStripeClient as unknown as Pick<StripeClient, "subscriptions">)
        ).resolves.toBeUndefined();
        expect(mockDoc.set).not.toHaveBeenCalled();
    });
