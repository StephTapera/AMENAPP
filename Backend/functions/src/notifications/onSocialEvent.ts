...
import { evaluatePolicies, PolicyEvaluation } from "./policies";
import { applyGrouping } from "./grouping";
import { sendPushNotification } from "./sendPush";
import { incrementUnseenCount } from "./counts";
 ...
 */
export const onFollowEvent = onDocumentCreated(
    "follows/{docId}",
    async (event) => {
        ...
        if (isPrivate) {
            ...
            const actor = await getActorInfo(followedId);
            if (!actor) return;

            const routes = buildRoutes(NotificationType.FollowRequestAccepted, {
                actorId: followedId,
            });
            ...
            const routes = buildRoutes(NotificationType.Follow, {
                actorId: followerId,
            });
...
export const onAmenCreated = onDocumentCreated(
    ...
    async (event) => {
        ...
        const routes = buildRoutes(NotificationType.Amen, {
            postId,
            actorId,
        });
...
async function handleComment(
    actor: { id: string; name: string; username: string; profileImageURL: string | null },
    actorId: string,
    postId: string,
    commentId: string,
    ...
    const routes = buildRoutes(NotificationType.Comment, {
        postId,
        commentId,
        actorId,
    });
...
async function handleReply(
    ...
    const routes = buildRoutes(NotificationType.Reply, {
        postId,
        commentId: replyId,
        parentCommentId,
        replyId,
    ...
    if (postAuthorId && postAuthorId !== recipientId && postAuthorId !== actorId) {
        const postAuthorRoutes = buildRoutes(NotificationType.Reply, {
            postId,
            commentId: replyId,
            parentCommentId,
            replyId,
        ...
        const postAuthorCandidate: NotificationCandidate = {
 ...
 */
export const onRepostCreated = onDocumentCreated(
    "reposts/{docId}",
    async (event) => {
        const snap = event.data;
        if (!snap) return;
        ...
        const routes = buildRoutes(NotificationType.Repost, {
            postId,
            actorId,
        });
 ...
 */
export const onPrayerSupportCreated = onDocumentCreated(
    "prayers/{prayerId}/supporters/{userId}",
    async (event) => {
        ...
        const routes = buildRoutes(NotificationType.PrayerSupported, {
            prayerId,
            actorId,
        });
        ...
        const candidate: NotificationCandidate = {
            recipientId,
            type: NotificationType.PrayerSupported,
            actorId,
            actorName: actor.name,
            actorUsername: actor.username,
            actorProfileImageURL: actor.profileImageURL,
 ...
 */
export const onPrayerAnswered = onDocumentUpdated(
    "prayers/{prayerId}",
    async (event) => {
        ...
        if (before.answered === true || after.answered !== true) return;

        const prayerId = event.params.prayerId;
        const authorId = after.userId || after.authorId;
        if (!authorId) return;

        const actor = await getActorInfo(authorId);
        if (!actor) return;
 ...
 */
export const onChurchNoteShared = onDocumentCreated(
    "churchNotes/{noteId}/shares/{userId}",
    async (event) => {
        ...
        const routes = buildRoutes(NotificationType.ChurchNoteShared, {
            noteId,
            actorId,
        });
        ...
        const candidate: NotificationCandidate = {
            recipientId,
            type: NotificationType.ChurchNoteShared,
            actorId,
            actorName: actor.name,
            actorUsername: actor.username,
            actorProfileImageURL: actor.profileImageURL,
...
export async function processCandidate(
    ...
    try {
        // 1. Policy evaluation
        const policy: PolicyEvaluation = await evaluatePolicies(candidate);

        if (policy.result === "suppress") {
            console.log(
                `Notification suppressed: ${candidate.type} from ${candidate.actorId} to ${candidate.recipientId} — reason: ${policy.triggeredBy}`
            );
            return;
        }

        // 2. Apply grouping — returns the notification doc ID (new or existing)
        const { notificationId, isNewNotification, notificationDoc } =
            await applyGrouping(candidate);
        ...
        if (isNewNotification) {
            badgeCount = await incrementUnseenCount(candidate.recipientId);
        }
        ...
        if (policy.pushAllowed && notificationDoc) {
            ...
            console.log(
                `Push sent for ${candidate.type}: ${pushResult.tokensSucceeded}/${pushResult.tokensAttempted} succeeded`
            );
        } else if (policy.result === PolicyResult.Digest) {
        ...
        console.error(
            `Error processing notification ${candidate.type} from ${candidate.actorId} to ${candidate.recipientId}:`,
            error
        );
    }
}
