...
export interface PolicyEvaluation {
    ...
    result: PolicyResult;
    /** Which policy triggered the decision (for logging/debugging). */
    triggeredBy: string | null;
    /** Whether push should be sent (false if downranked or in-app only). */
    pushAllowed: boolean;
    /** Whether sound should play on the push. */
...
function suppress(reason: string): PolicyEvaluation {
    return {
        result: PolicyResult.Suppress,
        triggeredBy: reason,
        pushAllowed: false,
        soundAllowed: false,
    };
...
function downrank(reason: string): PolicyEvaluation {
    return {
        result: PolicyResult.Downrank,
        triggeredBy: reason,
        pushAllowed: false,
        soundAllowed: false,
    };
...
function inAppOnly(reason: string): PolicyEvaluation {
    return {
        result: PolicyResult.InAppOnly,
        triggeredBy: reason,
        pushAllowed: false,
        soundAllowed: false,
    };
...
function digest(reason: string): PolicyEvaluation {
    return {
        result: PolicyResult.Digest,
        triggeredBy: reason,
        pushAllowed: false,
        soundAllowed: false,
    };
 ...
 */
function getLocalTimeInZone(
    now: Date,
    tz: string
): { hours: number; minutes: number; isSunday: boolean } {
    try {
 ...
 */
function checkSelfNotification(
    candidate: NotificationCandidate
): PolicyEvaluation {
...
async function checkBlocked(
    ...
    const blocked = await isBlocked(candidate.actorId, candidate.recipientId);
    if (blocked) {
...
async function checkMuted(
    ...
    const muted = await isMuted(candidate.recipientId, candidate.actorId);
    if (muted) {
 ...
 */
async function checkContentVisible(
    candidate: NotificationCandidate
): Promise<PolicyEvaluation> {
    // Check post visibility if notification references a post
    if (candidate.postId) {
        const visible = await isPostVisible(candidate.postId);
        if (!visible) {
    ...
    if (candidate.commentId) {
        const visible = await isCommentVisible(candidate.commentId);
        if (!visible) {
...
async function checkPrivateAccount(
    ...
    const isPrivate = await isPrivateAccount(candidate.recipientId);
 ...
 */
async function checkActorTrust(
    candidate: NotificationCandidate
): Promise<PolicyEvaluation> {
    const active = await isUserActive(candidate.actorId);
    if (!active) {
...
async function checkRateLimit(
    ...
    const recentNotifs = await db
        ...
        .limit(ACTOR_RATE_LIMIT_PER_HOUR + 1)
        .get();

    const isRateLimited = recentNotifs.size >= ACTOR_RATE_LIMIT_PER_HOUR;
...
async function checkRecipientPreferences(
    ...
    const prefs = await getUserPreferences(candidate.recipientId);
    const category = getCategoryForType(candidate.type);
...
async function checkQuietHours(
    ...
    if (!prefs.quietHoursEnabled) return ALLOW;
    if (!prefs.quietHoursStart || !prefs.quietHoursEnd) return ALLOW;
