...
export const archiveRoom = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "archiveRoom", 10);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
...
export const dismissSpaceAmbientSignal = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    await writeAudit(spaceId, uid, "dismissSpaceAmbientSignal", "ambientSignal", signalId);
    return { ok: true };
});
...
export const dismissSpaceMemoryNode = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    await batch.commit();
    await writeAudit(spaceId, uid, "dismissSpaceMemoryNode", "memoryNode", nodeId);
    return { ok: true };
});

export const deleteRoomPost = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "deleteRoomPost", 20);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
    const roomId = stringField(data, "roomId", 160);
...
export const dissolveEphemeralRoom = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "dissolveEphemeralRoom", 5);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
    const roomId = stringField(data, "roomId", 160);
    await requireMember(spaceId, uid, adminRoles);
    ...
    const jobRef = db.collection("spaces").doc(spaceId).collection("intelligence").doc();
    await jobRef.set({
        id: jobRef.id,
        spaceId,
        sourceRoomId: roomId,
        type: "ephemeral_room_summary_job",
        status: "queued",
        createdBy: uid,
        createdAt: now(),
        updatedAt: now(),
    ...
    await writeAudit(spaceId, uid, "dissolveEphemeralRoom", "room", roomId, { summaryJobId: jobRef.id });
    return { ok: true, summaryJobId: jobRef.id };
});
...
export const updateSpaceSettings = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "updateSpaceSettings", 10);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
    await requireMember(spaceId, uid, adminRoles);
...
export const updateSpaceDNA = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "updateSpaceDNA", 10);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
    await requireMember(spaceId, uid, adminRoles);
...
export const updateSpaceCovenant = onCall({ enforceAppCheck: true }, async (request) => {
    const uid = requireAuth(request);
    await enforceRateLimit(uid, "updateSpaceCovenant", 10);
    const data = dataMap(request.data);
    const spaceId = stringField(data, "spaceId", 160);
    await requireMember(spaceId, uid, adminRoles);
    await db.collection("spaces").doc(spaceId).set({
