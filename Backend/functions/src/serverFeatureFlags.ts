...
export interface ServerSafetyFlags {
    /** Whether the v2 moderation pipeline is active. */
    moderationV2Enabled: boolean;
    /** Whether enhanced DM content scanning is active. */
    dmEnhancedScanningEnabled: boolean;
    /** Whether the v2 anti-harassment engine is active. */
    antiHarassmentV2Enabled: boolean;
    /**
     * Whether the server-side messaging block enforcement trigger is active.
     * When false, the enforceMessageRestrictions trigger is a no-op.
     * Defaults to true — must be explicitly set to false to disable.
     */
    messagingBlockEnforcementEnabled: boolean;
    /**
     * Whether sending a message while blocked by the recipient triggers a
     * block attempt recording in enforcementHistory.
     */
    blockAttemptRecordingEnabled: boolean;
}
 ...
 */
const SAFE_DEFAULTS: ServerSafetyFlags = {
    moderationV2Enabled: true,
    dmEnhancedScanningEnabled: true,
    antiHarassmentV2Enabled: true,
    messagingBlockEnforcementEnabled: true,
...
let flagsCache: ServerSafetyFlags | null = null;
 ...
 */
export async function getServerSafetyFlags(): Promise<ServerSafetyFlags> {
    const now = Date.now();
    ...
    try {
        ...
        if (!snap.exists) {
            ...
            functions.logger.info(
                "[ServerFlags] system/serverFeatureFlags does not exist — using safe defaults."
            );
        ...
        const flags: ServerSafetyFlags = {
            moderationV2Enabled:
                typeof data.moderationV2Enabled === "boolean"
                    ? data.moderationV2Enabled
                    : SAFE_DEFAULTS.moderationV2Enabled,

            dmEnhancedScanningEnabled:
                typeof data.dmEnhancedScanningEnabled === "boolean"
                    ? data.dmEnhancedScanningEnabled
                    : SAFE_DEFAULTS.dmEnhancedScanningEnabled,

            antiHarassmentV2Enabled:
                typeof data.antiHarassmentV2Enabled === "boolean"
                    ? data.antiHarassmentV2Enabled
                    : SAFE_DEFAULTS.antiHarassmentV2Enabled,

            messagingBlockEnforcementEnabled:
                typeof data.messagingBlockEnforcementEnabled === "boolean"
                    ? data.messagingBlockEnforcementEnabled
                    : SAFE_DEFAULTS.messagingBlockEnforcementEnabled,

            blockAttemptRecordingEnabled:
                typeof data.blockAttemptRecordingEnabled === "boolean"
        ...
        functions.logger.error(
            "[ServerFlags] Failed to read system/serverFeatureFlags — using safe defaults.",
            err
        );
 ...
 */
export const invalidateServerFlagCache = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    const context = { auth: request.auth, app: request.app };
        ...
        functions.logger.info(
            `[ServerFlags] Cache invalidated by admin ${context.auth.uid}`
        );
