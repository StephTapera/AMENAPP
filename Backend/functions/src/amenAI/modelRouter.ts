...
export function routeAmenAIRequest(request: AmenAIRouteRequest): AmenAIRouteDecision {
  ...
  const requiresBackendForSensitivity = sensitive.some((category) =>
    backendOnlySensitiveCategories.has(category),
  );
  ...
  const mustStayBackend =
    backendOnlyTasks.has(request.taskType) ||
    requiresBackendForSensitivity ||
    riskTier === "high" ||
    riskTier === "restricted";
...
function defaultProviderFor(taskType: AmenAITaskType, directClientRequested: boolean): AmenAIProvider {
  if (
    directClientRequested &&
    ["daily_verse_quick_reflection", "church_note_title_summary", "post_caption_suggestion", "church_grounded_search", "tts_narration"].includes(taskType)
  ) {
    return "gemini";
  }
