...
function runTextModeration(text: string): {
    flagged: boolean;
...
async function algoliaIndexPost(
    postId: string,
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `Algolia PUT posts/${postId} failed: ${response.status} ${body}`
        );
    }
}
...
export const finalizePostOnCreate = onDocumentCreated(
    ...
    if (!data) {
        logger.error(`[onPostCreated] Empty snapshot for post ${postId}`);
        return;
    }
    ...
    if (!authorId) {
        logger.warn(`[onPostCreated] Post ${postId} has no authorId — skipping`);
        return;
    }
        ...
        if (modResult.flagged) {
            logger.warn(
                `[onPostCreated] Post ${postId} by ${authorId} flagged — ` +
            ...
            if (modResult.autoBan) {
                ...
                try {
                    ...
                    logger.warn(
                        `[onPostCreated] Author ${authorId} suspended — post ${postId} ` +
                        `matched category=${modResult.category}`
                    );
                } catch (e) {
                    logger.error(
                        `[onPostCreated] Failed to suspend author ${authorId}`, e
                    );
                }
        ...
        const currentStatus: string = data.status ?? "published";
        ...
        if (currentStatus === "publishing" || !data.publishedAt) {
            try {
                ...
                logger.error(
                    `[onPostCreated] Failed to update status for post ${postId}`, e
                );
            }
        }
        ...
        if (algoliaKey) {
            const record: Record<string, unknown> = {
                ...
                createdAt: data.createdAt
                    ? (data.createdAt as admin.firestore.Timestamp).toMillis?.() ?? Date.now()
                    : Date.now(),
            ...
            try {
                ...
                logger.info(
                    `[onPostCreated] Indexed post ${postId} in Algolia`
                );
            } catch (e) {
                ...
                logger.error(
                    `[onPostCreated] Algolia index failed for post ${postId} — ` +
                    `search unavailable for this post until re-indexed`, e
                );
            }
        } else {
            logger.warn(
                `[onPostCreated] ${ALGOLIA_WRITE_KEY_SECRET} not configured — ` +
        ...
        logger.info(
            `[onPostCreated] Finalized post ${postId} — ` +
