...
export interface GuardrailAuditResult {
  ...
  patchedAnswer?: string;     // Only present if the answer was modified
  leadershipPromptRequired: boolean;
}
...
export function auditResponse(
  response: BereanStructuredResponse,
  classification: SpiritualStateClassification | undefined
): GuardrailAuditResult {
  const flags = new Set<SensitivityFlag>(response.sensitivityFlags);
  let answer = response.answer;
  let patchedAnswer: string | undefined;
  ...
  if (
    response.doctrinalConfidence < 0.6 &&
  ...
  const leadershipPromptRequired =
    flags.has("pastoral_escalation") ||
 ...
 */
export async function createLeadershipReferral(
  userId: string,
  ...
  const referral: LeadershipReferral = {
    id: referralId,
    userId,
 ...
 */
export function buildEscalationContextSummary(
  primaryTopic: string,
