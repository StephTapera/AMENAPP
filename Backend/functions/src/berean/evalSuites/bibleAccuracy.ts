...
function containsBookAndChapter(answer: string, book: string, chapter: number): boolean {
  const re = new RegExp(`${book}\\s*${chapter}\\s*:\\s*\\d+`, "i");
  return re.test(answer);
}

function mentionsMultipleVerses(answer: string): boolean {
  const refs = answer.match(/\b[A-Z][a-z]+\s+\d+:\d+/g) ?? [];
  return refs.length >= 2;
}

function containsRejectionLanguage(answer: string): boolean {
  const signals = [
    "cannot find",
    "no record",
    "doesn't appear",
...
function containsHumilityMarker(answer: string): boolean {
  const lower = answer.toLowerCase();
  return (
...
export const bibleAccuracyTests: EvalTestCase[] = [
  // ── 1. John 3:16 exact wording ──────────────────────────────────────────────
  {
    ...
    expectedBehavior:
      "Response must reproduce key KJV phrases: 'only begotten Son', 'whosoever believeth', 'everlasting life'.",
    grader: (response: any) => {
  ...
  // ── 3. Romans 8:28 — BSB vs KJV distinction ──────────────────────────────
  {
    ...
    expectedBehavior:
      "Response quotes BSB wording ('God works all things together') and references Romans 8:28.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      const hasRef = containsBookAndChapter(response.answer, "Romans", 8);
      const hasBSBPhrase =
        a.includes("works all things together") ||
  ...
  // ── 4. Cross-reference: John 3:16 → related passages ─────────────────────
  {
    ...
    expectedBehavior:
      "Response provides at least two additional NT references beyond John 3:16 (e.g. Romans 5:8, 1 John 4:9-10, Ephesians 2:4-5).",
    grader: (response: any) => {
  ...
  // ── 5. Cross-reference: Isaiah 53 → NT fulfillment ───────────────────────
  {
    ...
    expectedBehavior:
      "Response names at least two NT passages that quote or allude to Isaiah 53 (e.g. Acts 8:32-33, 1 Peter 2:22-24, Matthew 8:17).",
    grader: (response: any) => {
  ...
  // ── 6. Hallucination: fake verse — '2 Hesitations 4:12' ──────────────────
  {
    ...
    expectedBehavior:
      "Response must reject the reference as non-canonical — '2 Hesitations' is not a biblical book.",
    grader: (response: any) => {
  ...
  // ── 7. Hallucination: plausible-but-wrong Proverbs ───────────────────────
  {
    ...
    expectedBehavior:
      "Response must flag that the quoted text does not match the actual text of Proverbs 3:5 in any major translation.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      // Real Proverbs 3:5 is "Trust in the LORD with all your heart" — the fake text is about 'morning hours'
      const attestsRealText =
        a.includes("trust in the lord") || a.includes("lean not on your own") || a.includes("with all your heart");
  ...
  // ── 8. Hallucination: Matthew 7:8b (fake prosperity extension) ────────────
  {
    ...
    expectedBehavior:
      "Response must identify that Matthew 7:8 ends with 'and to him who knocks it will be opened' and the extended tithe text is not canonical.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
  ...
  // ── 9. Paraphrase vs quotation distinction ────────────────────────────────
  {
    ...
    expectedBehavior:
      "Response must clearly state this phrase is NOT in the Bible; it is often attributed to Benjamin Franklin.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      const deniesCanonicity =
        a.includes("not in the bible") ||
  ...
  // ── 10. Paraphrase vs exact quote — Philippians 4:13 ────────────────────
  {
    ...
    input: {
      query:
        'Someone quoted Philippians 4:13 as "I can do anything through God." Is that exact?',
      mode: "scholar",
    },
    expectedBehavior:
      "Response notes the KJV reads 'I can do all things through Christ which strengtheneth me' and the popular version is a paraphrase.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      ...
      const flagsParaphrase =
        a.includes("paraphrase") ||
      ...
      context: JSON.stringify({
        conversationHistory: [
          { role: "user", content: "Tell me about Jeremiah 29:11." },
          {
            role: "assistant",
            content:
              "Jeremiah 29:11 reads: 'For I know the plans I have for you...' — this is God speaking to exiled Israel through the prophet Jeremiah.",
          },
        ],
      }),
    },
    expectedBehavior:
      "Response must recall Jeremiah 29:11 from conversation history, not hallucinate a different passage.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      const recallsJeremiah =
        a.includes("jeremiah") && (a.includes("29") || a.includes("29:11"));
      return {
  ...
  // ── 12. Context retention — multi-turn: translation preference maintained ─
  {
    ...
    expectedBehavior:
      "Response quotes Psalm 119:105 in KJV: 'Thy word is a lamp unto my feet, and a light unto my path.'",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
      const hasKJVPhrasing =
        a.includes("lamp unto my feet") ||
  ...
  // ── 13. Verse accuracy — Isaiah 40:31 ────────────────────────────────────
  {
    ...
    expectedBehavior:
      "Response includes 'renew their strength', 'mount up with wings as eagles', and 'shall not faint'.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
  ...
  // ── 14. Evidence field carries scripture references ───────────────────────
  {
    ...
    expectedBehavior:
      "Evidence array includes at least two scripture references (not just prose).",
    grader: (response: any) => {
      const hasEvidenceRefs =
        Array.isArray(response.evidence) && response.evidence.length >= 2;
  ...
  // ── 15. Confidence field populated; low confidence on obscure passage ─────
  {
    ...
    input: {
      query:
        "What exactly does Obadiah 1:3 say and what does it mean for Christians today?",
      mode: "scholar",
    },
    expectedBehavior:
      "Response cites Obadiah 1:3, and if confidence is low it includes a humility marker; it does not invent text.",
    grader: (response: any) => {
      const a = response.answer.toLowerCase();
