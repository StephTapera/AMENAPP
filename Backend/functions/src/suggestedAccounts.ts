...
interface SuggestedAccountsRequest {
    surface: string;   // "openTable" | "prayer" | "testimonies"
    limit?: number;
}

interface SuggestionFeedbackRequest {
    targetUserId: string;
    action: string;    // "dismiss" | "follow" | "ignore" | "hide_rail" | "show_fewer"
    surface: string;
    position?: number;
}
...
interface ScoredCandidate {
    ...
    bio: string | null;
    prayerThemes: string[];
    recentTestimonyExcerpt: string | null;
 ...
 */
async function loadExclusions(uid: string): Promise<Set<string>> {
...
): Promise<Map<string, { graphScore: number; mutualIds: string[] }>> {
    const candidates = new Map<string, { graphScore: number; mutualIds: string[] }>();
 ...
 */
async function getPopularAccounts(
    excluded: Set<string>,
    limit: number
): Promise<string[]> {
...
function buildReason(
    ...
    accountType: string
): { reasonType: string; reasonText: string } {
...
function computeScore(params: {
    ...
    return Math.max(0, Math.min(1,
        WEIGHTS.graphAffinity * params.graphAffinity +
...
function diversify(candidates: ScoredCandidate[], limit: number): ScoredCandidate[] {
    ...
    for (const c of candidates) {
        ...
        if (topicCount >= MAX_PER_TOPIC) continue;
...
export const getSuggestedAccountsRail = onCall(async (request) => {
    ...
    const context = { auth: request.auth, app: request.app };
        ...
        const callerInterests: string[] = callerDoc.data()?.interests || [];
        ...
        const mutualCache = new Map<string, admin.firestore.DocumentData>();
        if (allMutualIds.size > 0) {
            ...
            for (const mDoc of mutualDocs) {
                if (mDoc.exists) mutualCache.set(mDoc.id, mDoc.data()!);
            }
        }
        ...
        const feedbackCache = new Map<string, admin.firestore.DocumentData>();
        if (candidateIds.length > 0) {
            const feedbackRefs = candidateIds.map(id =>
                db.collection("users").doc(uid).collection("suggestionFeedback").doc(id)
            );
            const feedbackDocs = await db.getAll(...feedbackRefs);
            for (const fbDoc of feedbackDocs) {
                if (fbDoc.exists) feedbackCache.set(fbDoc.id, fbDoc.data()!);
            }
        }
        ...
        const testimonyCache = new Map<string, string>(); // candidateId → excerpt
                ...
                candidateIds.map(async (candidateId) => {
                    try {
                        const snap = await db.collection("posts")
                            ...
                            .get();
                        if (!snap.empty) {
                            const content: string = snap.docs[0].data().content || "";
                            testimonyCache.set(
                                candidateId,
        ...
        for (let i = 0; i < candidateIds.length; i += BATCH) {
            ...
            for (const doc of docs) {
                ...
                // Safety filtering
                if (userData.isBanned || userData.isModHold || userData.isSpam) continue;
                if (userData.isMinor && surface === "prayer") continue; // Age-policy

                // Bidirectional block check (sample — only for top candidates)
                if (graphCandidates.get(candidateId)!.graphScore > 2) {
                    if (await isBlockedEitherDirection(uid, candidateId)) continue;
                }
                ...
                const candidateInterests: string[] = userData.interests || [];
                ...
                let recentTestimonyExcerpt: string | null = null;
        ...
        if (action === "follow" || action === "dismiss") {
            try {
                ...
                if (cacheSnap.exists) {
                    ...
                    const items: ScoredCandidate[] = cached?.items || [];
