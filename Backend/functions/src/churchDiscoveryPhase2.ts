...
export const refreshChurchLiveState = onCall({ enforceAppCheck: true, region: REGION }, async (request) => {
...
export const generateChurchExperienceSummary = onCall({ enforceAppCheck: true, region: REGION }, async (request) => {
    requireAppCheck(request as CallableRequest);
    const churchId = readChurchId(request as CallableRequest);
...
export const calculateChurchFitScore = onCall({ enforceAppCheck: true, region: REGION }, async (request) => {
    requireAppCheck(request as CallableRequest);
...
export const resolveChurchSmartAction = onCall({ enforceAppCheck: true, region: REGION }, async (request) => {
    requireAppCheck(request as CallableRequest);
    const uid = requireAuth(request as CallableRequest);
    const churchId = readChurchId(request as CallableRequest);
...
export const generateBereanChurchSuggestions = onCall({ enforceAppCheck: true, region: REGION }, async (request) => {
    requireAppCheck(request as CallableRequest);
