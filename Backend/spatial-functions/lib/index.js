...
exports.onCovenantRoomMessageCreatedUpdateAmbientState = exports.backfillCovenantSpatialRooms = exports.createCovenantSpatialRoom = exports.generateSpatialRoomTheme = void 0;
...
function requireUid(uid) {
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in before managing spatial rooms.");
    }
    return uid;
}
function readString(data, key, maxLength) {
    ...
    if (!trimmed || trimmed.length > maxLength) {
        throw new https_1.HttpsError("invalid-argument", `${key} must be 1-${maxLength} characters.`);
    }
    return trimmed;
}
function readBoolean(data, key, fallback = false) {
    const value = data[key];
...
function readKind(data) {
    ...
    if (typeof value !== "string" || !validKinds.has(value)) {
        throw new https_1.HttpsError("invalid-argument", "kind is not a supported spatial room kind.");
    }
    return value;
}
...
async function assertCanManageCovenant(uid, covenantId) {
    const covenantSnap = await db.collection("covenants").doc(covenantId).get();
    if (!covenantSnap.exists) {
        throw new https_1.HttpsError("not-found", "Covenant was not found.");
    }
    ...
    if (!membership || !managerRoles.has(role) || !["active", "trialing"].includes(status)) {
        throw new https_1.HttpsError("permission-denied", "Only covenant creators, admins, and moderators can manage spatial rooms.");
    }
}
function roomTypeForKind(kind) {
    switch (kind) {
...
function motionStyleForKind(kind) {
    switch (kind) {
...
async function uploadGeneratedImage(covenantId, roomName, base64Image) {
...
async function generateSpatialMedia(draft) {
    ...
    try {
        ...
        firebase_functions_1.logger.warn("Spatial room media generation failed", {
...
function buildTheme(draft, media) {
...
function buildAmbientState(draft, activeCount = 0) {
...
function draftFromRoom(room) {
    ...
    const kind = type === "prayer" ? "prayer" : type === "study" ? "bibleStudy" : type === "q_and_a" ? "berean" : type === "events" ? "event" : "community";
    return {
...
exports.generateSpatialRoomTheme = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    const uid = requireUid(request.auth?.uid);
    const data = request.data;
    const draft = readDraft(data);
    await assertCanManageCovenant(uid, draft.covenantId);
    const media = data.media;
    const suppliedMedia = typeof media === "object" && media !== null ? media : undefined;
...
exports.createCovenantSpatialRoom = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    const uid = requireUid(request.auth?.uid);
    const draft = readDraft(request.data);
    await assertCanManageCovenant(uid, draft.covenantId);
...
exports.backfillCovenantSpatialRooms = (0, https_1.onCall)({ secrets: [openAIKey], timeoutSeconds: 540 }, async (request) => {
    const uid = requireUid(request.auth?.uid);
    const data = request.data;
    ...
    const requestedRoomIds = Array.isArray(roomIdsValue)
        ? roomIdsValue.filter((value) => typeof value === "string" && value.trim().length > 0).slice(0, 100)
        : [];
...
exports.onCovenantRoomMessageCreatedUpdateAmbientState = (0, firestore_1.onDocumentCreated)("covenants/{covenantId}/rooms/{roomId}/messages/{messageId}", async (event) => {
    ...
    const theme = room.spatialTheme;
    const kind = typeof theme?.kind === "string" && validKinds.has(theme.kind) ? theme.kind : draftFromRoom({ ...room, covenantId }).kind;
    const isPrayer = kind === "prayer";
    const isVoice = Boolean(room.voice?.enabled);
