...
| ID | Lane | Severity | Title | Why Gated | Exact Action | Affected Files | Est. | Blocking |
|---|---|---|---|---|---|---|---|---|
| D-IDENTITY-001 | 🟡 | P0 | Deploy trust-scoring identity-invariant fix | Source now blocks cross-user `data.userId` targeting in trust scoring, but Cloud Functions deploy is external/live. | After `npm --prefix Backend/functions run build` and targeted tests pass, deploy only the affected creator functions to the intended region; do not run broad deploy. | `Backend/functions/src/globalResilience/trustScoring.ts` | 30 min | Yes |
| D-RULES-TEST-001 | 🟡 | P0 | Run backend rules emulator suite | Rules and storage hardening cannot be considered live-safe without emulator verification. | `cd "/Users/stephtapera/Desktop/AMEN/AMENAPP copy" && firebase emulators:exec --project amen-5e359 --only firestore,functions,storage "cd Backend/rules-tests && npm test"` | `firestore.rules`, `storage.rules`, `Backend/rules-tests/` | 45-90 min | Yes |
| D-ADMIN-001 | 🟡/🔴 | P1 | Replace user-doc admin mirrors with custom claims or approve policy | Several backend admin gates still consult `users/{uid}.isAdmin`; Module D requires server-verified custom claims as authority. | Decide whether user-doc mirror is intentionally server-only; otherwise patch callables to use `request.auth.token.admin` / Admin SDK custom claims and deploy per-function. | `Backend/functions/src/intelligence/digestBuilder.ts`, `Backend/functions/src/globalResilience/crisisBulletins.ts` | 1-2 hr | No |
